...
**Cơ chế hai pha** *(sửa lớn PHA-2 chốt 2026-07-12 — xem changelog Math §0).* `D = min(1001, ⌊pot/1_000_000⌋)`.
- **PHA-1 — "Giai đoạn Ngày" (ngày 1 → 1001):** LAMP khoá "có-điều-kiện" (chưa sở-hữu). Số dư LAMP **sinh MAGIC** (quyền tiêu dịch-vụ) — engine chỉ ĐỌC số dư, không đụng/không đốt LAMP. User hưởng = dòng MAGIC hằng ngày. Ngày nào không tiêu ≥ `MIN_MAGIC_CONSUME` qua dịch-vụ **trả phí** → thu-hồi 1 LAMP về pot (anti-idle). *(giữ nguyên mô-hình cũ.)*
- **PHA-2 — "Giai đoạn Kỳ" (từ ngày 1002; 1 Kỳ = 5 ngày = 432.000 slot):** nhịp chuyển từ NGÀY sang KỲ, mỗi Kỳ xử-lý ĐÚNG một lần: **Kỳ active** (tiêu đủ) → chuyển `q = min(5, c)` LAMP **RA THẲNG ví Phoenix của user** = sở-hữu-hoàn-toàn tức-thời (5 = 1 LAMP/ngày × 5 ngày); **Kỳ idle** → thu-hồi `q = min(5, c)` LAMP về pot NGAY Kỳ đó. Phần đã ra ví = bất-khả-xâm-phạm. Đóng+burn vault khi hết LAMP điều-kiện (`c = 0`).

> **Khác mô-hình cũ (2 điểm):** (1) giao-quyền-sở-hữu = **chuyển RA ví Phoenix ngoài vault**, KHÔNG còn "vest ghi-sổ trong vault + user bấm [Rút]" (ClaimVested); (2) thu-hồi PHA-2 theo **TỪNG Kỳ idle** (tức-thời), KHÔNG còn chờ 1001-Kỳ-idle. Đơn giản hơn cho user, pot tái-tuần-hoàn nhanh hơn.
...
|---|---|---|---|
| **Q1** | **Model 2-PHA: có-điều-kiện → sở-hữu-ra-ví theo Kỳ** (thay borrow-only v3; PHA-2 5-LAMP/epoch chốt 07-12) | (a) tạo hợp-đồng-trung-thành: kiên-trì 1001 ngày → sở-hữu thật; (d) người bỏ-cuộc nuôi người-mới. Bền hơn borrow-only (không điểm-thưởng) và vesting-bán-ngay (cung tức-thời). LAMP-sở-hữu vào THẲNG ví Phoenix (bất-khả-xâm), thu-hồi per-Kỳ-idle tái-tuần-hoàn nhanh. | Vòng đời user rất dài. So mô-hình cũ: bỏ ClaimVested + trạng-thái vested-in-vault ⟹ ĐƠN-GIẢN hơn (4 redeemer thay 5). |
...
| **Q3** | **Cung-bán hoãn qua rào cam-kết** (không cung-tức-thời) | (a)(b) chỉ user tiêu-thật đủ 1001 ngày mới có LAMP-sống-sót để bán; sở-hữu-ra-ví nhỏ-giọt (≤5/Kỳ = 1/ngày). Cung rải đều ≥1001 ngày, không sốc. | Người-dùng thật phải chờ rất lâu mới sở-hữu — trải-nghiệm "chậm được thưởng". |
| **Q4** | **Người bỏ-cuộc nuôi người-mới** (pot tự-nuôi phản-chu-kỳ) | (a)(d) anti-idle PHA-1 + ReclaimEpoch PHA-2 (per-Kỳ-idle) + phí-thu-bằng-LAMP-theo-giá-trị (giá xuống → thu nhiều LAMP → pot phình đúng lúc thị-trường lạnh). LAMP về pot = kế-toán, KHÔNG đốt. | PHA-2 LAMP-sở-hữu RỜI-HỆ sang ví user → pot cần nguồn bù đủ. Rủi-ro pot-cạn thật hơn v3 (xem R1). |
...
|---|---|---|---|
| **R1** | **Pot cạn khi nhiều user qua PHA-2 cùng lúc** (LAMP-sở-hữu rời-hệ, không quay lại pot) | 🟡 TRUNG (rủi-ro tham-số, không phải lỗi thiết-kế) | 3 nguồn nạp: phí-theo-giá-trị (phản-chu-kỳ) + anti-idle/ReclaimEpoch + Treasury-topup. **[Cần mô-phỏng]** cân-đối tốc-độ-sở-hữu-ra vs tốc-độ-nạp; ngưỡng treasury-topup. |
...
| **R6** | **Thu-hồi PHA-2 per-Kỳ-idle (min(5,c)/Kỳ)** — mô-hình mới thu-hồi phần chưa-kiếm NGAY từng Kỳ idle (khác cũ chờ 1001-Kỳ ≈ 13.7 năm) | 🟢 THẤP (đã đơn-giản hoá 07-12) | Chỉ thu phần CHƯA-kiếm (`conditional`), KHÔNG đụng phần đã ra ví Phoenix. Pot tái-tuần-hoàn nhanh. Rủi-ro cũ "đọng LAMP 13.7 năm" đã hết. |
...
- **B1 — Engine Gen đọc-số-dư (MAGIC/CARP-team).** Chỉ nối Gen production sau khi MAGIC/CARP-team spell-out validator on-chain đọc-số-dư (KHÔNG spend/đốt LAMP — mẫu cũ fire→Treasury bị cấm tái dùng). Validator vault khoá+sở-hữu-ra-ví thuần build/chạy độc-lập, không cần chờ Gen.
...
- **B4 (phụ) — GreenBack settlement + shadow-price (CARP-team); release LAMP kho→pot + phí-bằng-LAMP (LAMP-team)** phải sẵn sàng trước khi bật GetMAGIC + phản-chu-kỳ production.

---

## 5. Lộ-trình mốc

**Nguyên-tắc thứ-tự:** phần khoá+sở-hữu-ra-ví thuần build/test ĐỘC-LẬP trước (không cần Gen) → spell-out engine Gen → nối Gen → chuẩn Registry + bật anti-idle → GreenBack → phản-chu-kỳ.
...
| **M7** | fee_refill_lamp phản-chu-kỳ + mô-phỏng cân-đối dòng-sở-hữu-ra (R1) | LAMP/backend |
...
| **Q-B** | **Nhịp: PHA-1 theo NGÀY, PHA-2 theo KỲ (5 ngày).** Anti-idle (Ngày) tick 1 LAMP/ngày; OwnEpoch/ReclaimEpoch (Kỳ) `q=min(5,c)`. MAGIC drip có thể daily-buffer ở backend. | PHA-1 daily + PHA-2 epoch (đã chốt 07-12); MAGIC drip-epoch mặc-định, buffer-daily backend. | Ảnh-hưởng cảm-giác user + độ phức-tạp on-chain. |
| **Q-C** | **~~Forfeit 1001 epoch~~ ĐÃ GIẢI (07-12):** thu-hồi PHA-2 nay per-Kỳ-idle `min(5,c)`, không còn ngưỡng 1001-Kỳ. | (không còn câu hỏi — đã đơn-giản hoá). | — |
| **Q-D** | **`MIN_MAGIC_CONSUME` = 10% gen-able + granularity** (PHA-1 per-Ngày, PHA-2 per-Kỳ). | 10% daily-gen-able từ `conditional_lamp`; PHA-2 tính cumulative trong Kỳ. | Tham-số TẠM (maintainer đặt 07-06); sub-open, buộc với nhịp Q-B. |
| **Q-E** | **Xác nhận PHA-2-sở-hữu-ra-ví khớp WP §7.2** — đọc "cung-hoãn-qua-cam-kết 1001 ngày + nhỏ-giọt ≤5/Kỳ" là HỢP-LỆ, khác "cung-tồn-đọng chực-bán". | Hợp-lệ (đã qua rào cam-kết + sở-hữu-ra-ví nhỏ-giọt). | Diễn-giải whitepaper — cần maintainer xác nhận vì "sở-hữu-ra-ví" là directive Wakeme-layer, WP không spell-out. |

---

## 7. Điều-kiện-tiên-quyết chặn production (GO/NO-GO)

- **Whitepaper-gap khai-báo:** "GetLAMP / pot / 2-pha / sở-hữu-ra-ví-sau-1001-ngày / phí-thu-bằng-LAMP" KHÔNG có trong whitepaper — là **directive Wakeme-layer nội-bộ**. WP §7.2/§7.3 + /CARP hậu-thuẫn **nguyên-lý Gen-đọc-số-dư** rõ ràng; /CARP-team chịu trách-nhiệm spell-out cơ-chế on-chain (giao ở B1, §4).
...
- **Gaming đã sạch phần on-chain:** self-consumption (GV) hợp-lệ + khuyến-khích; sybil-đa-địa-chỉ vô-ích (D per-PersonDID *một-khi* uniqueness anchor thoả); catch-up-burst chặn (per-epoch cap `q=min(5,c)` + đơn-điệu `e>te` ở `own_epoch_ok`); không phải Terra (GreenBack có-back). Nhưng KHÔNG "hết lỗ" — 2 điều-kiện trên + GV5 (hút-yield tối-thiểu, kinh-tế) còn HỞ tới khi PA2+Registry land.
