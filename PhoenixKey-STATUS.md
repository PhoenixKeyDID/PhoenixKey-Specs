...
| ~~PA2 — UniquenessThread hai-validator~~ **LOẠI VĨNH VIỄN**, thay bằng **PC** — ✅ **đã land + đã nối** 2026-08-10 | — | — | PA2-hai-validator vô-nghiệm fixed-point hash (mỗi script bake hash của script kia). PC gộp một multi-purpose validator, `anchor_policy ≡ thread_policy ≡ own_policy`. K=32 (không phải 256 — bootstrap nguyên-tử K=256 ≈ 34 KB, vượt trần tx 16 KB). Đặc-tả §5.7ter. **⚠ "land + nối" là TRONG MÃ, chưa lên chuỗi** — xem dòng dưới |
| 🔴 **Cổng uniqueness PA2 chưa TỒN TẠI trên chuỗi** (đo 2026-08-17) | Anchorme, Wakeme (B3), mọi thứ tựa vào khử-trùng DID | on-chain (chờ chốt seed) | Chuỗi suy luận không có mắt lỏng: `uniqueness_bootstrap_seed` **chưa chọn** ⇒ `ValidatorParams` không apply được ⇒ **không tồn tại** policy-id đã-apply ⇒ không gì đúc được dưới cổng PA2. Bằng chứng: `deploy/plutus-preprod.json` `_stale:true` + `_stale_reason` tự khai *"taad CHUA apply param… Chi apply lai duoc sau khi chon seed UTxO that"*; `deploy/deploy_to_preprod.sh:123-151` fail-closed ⇒ đường deploy chính thống **chưa chạy một lần nào**; `git log --all -S'uniqueness_bootstrap_seed' -- deploy/` = 4 commit, **không commit nào chọn seed**; **0** tệp `.bootstrap` ở mọi nhánh. Không phải sự cố — là quyết định *(B) hoãn* (`_Agents/topics/dot-ton-dong-2026-08-14.md:301`): deploy trước khi PA2 có mã = đổi hash hai lần. **Hệ quả nặng hơn:** 2 anchor DID đang sống thật trên Preview nằm dưới policy `0f665f9967e5b735949e4def618b6b56cff9e18f0f74571303f49a3f` — **không có cổng uniqueness nào** — và PA-1 sẽ khác script-hash ⇒ 2 UTxO đó **không spend được** bằng validator mới |
...
| 🔴 **ServiceDID / AgentDID — validator đủ, backend và UI bằng 0** (đo 2026-08-18) | mục tiêu (3) | backend + app | `types.ak:53-63` có đủ 10 `EntityType` (Service là literal; **AgentDID = biến thể `AI`**, `state_nft_logic.ak:112`); `can_own` cho Person/Org sinh cả Service lẫn AI; `rust_core/src/taad_did.rs:1799` `build_create_child_taad_utxo_tx` là tx-builder THẬT, export FFI `lib.rs:576`. Nhưng **0 tệp Java** gọi `GenesisChild`, **0 màn hình** Dart/Frontend gọi hàm FFI đó. Mã đúng, chưa ai nối |
| 🟡 **OrgDID hôm nay KHÔNG phải anchor TAAD** (đính chính mục 5) | mục tiêu (2) | backend | `POST /identity/org/create` chạy được, nhưng `OrgServiceImpl.java:103-104` tự khai *"Publish minimal Org Document via metadata-6789 (transitional — swaps to TAAD UTxO mint redeemer when validator ships)"* ⇒ không mint state-NFT, không qua `can_own`/PoP-bind. Bảng mục 5 ghi "ĐƯỢC — không có chỗ đứt" là **thiếu chính xác** |
...
**Chưa có code:** 🔴 `did_subaddr.ak` (L3 unlinkable, chờ chốt [DEP-2]); 🔴 `did_stake.ak` (stake theo-DID). — `limit_meter.ak` anti-drain đã **gỡ khỏi danh sách này 2026-08-08**, xem dòng blocker đã gạch ở mục 2: tệp có thật trên `main` (`lib/phoenixkey/limit_meter.ak` 690 dòng + `validators/limit_meter_vault.ak`, hash `f3be6d6d…`). 🟡 I-CURVE-5 chưa enforce builder; kho bí-mật/phả-hệ seed chưa hợp-nhất; export re-key UI chưa cắm mặc-định; guardian nâng-cao (trọng-số/veto/cap) Todo; chứng-thực VeData-Glint/Midnight chờ VeData. ⚪ legacy-migration, on-ramp mandate, pool-ops (KES/VRF) build-ready-Todo.
