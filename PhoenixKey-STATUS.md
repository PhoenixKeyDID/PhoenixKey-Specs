...
>
> Cập nhật: 2026-08-15. Nguồn: audit per-module + đối chiếu code/CI thật; mục 5 đo lại đầu-cuối bằng `aiken check`/`aiken build` và gọi thật máy chủ đang chạy. Đợt 15/08 bổ sung: đọc chip giấy tờ (eMRTD), đầu dò tầm với, CORS tra-tên-công-khai, SDK username — **cả bốn đều là PR chưa merge**, xem mục 4.
...
| 2026-08-15 | Core (đọc chip giấy tờ) | eMRTD theo ICAO Doc 9303: `rust_core/src/emrtd/` **5.866 dòng / 102 test đơn-vị** (MRZ 11 · BAC 10 · SM 10 · LDS 35 · PA 25 · status 10 · transport 1). Cầu NFC gốc: Kotlin 217 dòng + Swift 276 dòng + Dart `EmrtdNfcTransport` 123 dòng / 10 test. CI **4/4 pass** (`rust_core` 2m34s · Flutter 1m15s). PR Core **#73 — chưa merge**. ⚠ **hai nửa chưa nối**: trong Rust `ApduTransport` mới có đúng một bản `ScriptedTransport` (giả lập cho test, `emrtd/transport.rs:74`) — chưa có bản gọi ngược xuống Dart ⇒ **chưa lần nào đọc thẻ thật** |
...
| **Chạm thẻ CCCD / hộ chiếu vào điện thoại để tự lấy số định danh** (khỏi gõ tay) | **CÓ CODE, CHƯA NỐI ĐẦU-CUỐI** (2026-08-15) | Chip **có** trả số định danh trong DG1 (MRZ) — không phải gõ tay. Phía Rust đọc + kiểm được đủ chuỗi ICAO 9303 (102 test); cầu NFC gốc Android/iOS + Dart có. Chỗ đứt: **không có bản `ApduTransport` nối Rust ↔ Dart**, nên chưa lần nào chạm thẻ thật. Còn một quyết-định thiết-kế chờ chốt: FFI callback từ Dart xuống, hay để Rust gọi thẳng Kotlin/Swift. ⚠ Khoá mở BAC dựng từ **số giấy tờ + ngày sinh + ngày hết hạn** (Doc 9303-11 Appendix D.2, có test vector TD1 trong `mrz.rs:533`) ⇒ vẫn cần **một lần** quét OCR hoặc nhập tay ba trường đó ở mặt có MRZ trước khi chạm chip. Riêng hành vi thật của CCCD Việt Nam **chưa đo trên thẻ thật** — mọi số ở đây là test vector ICAO, không phải phép thử thực địa |
