...
>
> Cập nhật: 2026-08-14 (đợt 12/08 đo lại ngày 14/08 — hai dòng đã đổi vì `main` các kho tiến tiếp). Nguồn: audit per-module + đối chiếu code/CI thật; mục 5 đo lại đầu-cuối bằng `aiken check`/`aiken build` và gọi thật máy chủ đang chạy.
>
...
| 🟡 **Khoá thiết bị — yếu-tố-2 khi chi từ ví Phoenix** | Rebirthme (ví Phoenix), Anchorme (datum genesis) | app | Aiken đã ép 2-of-2 (`auth_logic.ak:37-58`). **Tầng `rust_core` nay ĐÃ CÓ** (Core PR #51 gộp 2026-08-13): `rust_core/src/device_key.rs` sinh khoá Ed25519 + bọc envelope, FFI `taad_device_key_generate`/`taad_device_key_sign`, và `encode_taad_datum_create_v2` đặt `device_pkh` vào field 14 — đo 2026-08-14: `git grep -c device_pkh origin/main -- rust_core/src/*` cộng lại = **66** (dòng cũ ghi `= 0` nay sai). **Còn thiếu:** nối vào luồng dựng tx (chú thích `taad_did.rs:807` ghi rõ "chưa nối vào luồng dựng tx nào") và toàn bộ tầng ứng dụng — `git grep -l 'taad_device_key|deviceKey|device_pkh' origin/main -- 'lib/*' 'test/*'` → **0 tệp**. Phải Ed25519/secp256k1 — P-256 không verify được on-chain |
...
| **Chi tiền từ ví Phoenix** | **KHÔNG** | Core PR #51 **đã gộp 2026-08-13** ⇒ `device_pkh` nay có thật trên `main` (66 lần trong `rust_core/src`, đo 14/08) — dòng "chưa gộp, grep = 0" của đợt 12/08 đã hết đúng. Cái còn chặn đã đổi loại: `rust_core` có sinh khoá + ký + mã hoá datum, nhưng **chưa nối vào luồng dựng tx** (`taad_did.rs:807`) và **tầng ứng dụng chưa gọi** (`lib/`+`test/` → 0 tệp). Vẫn thiếu hàm dựng+ký giao dịch 2-chữ-ký đầu-cuối |
| **Khôi phục trên máy thứ hai ra đúng DID cũ** | **KHÔNG** | `did` gấp 32 byte ngẫu nhiên **và** một mốc thời gian trên chuỗi vào tiền-ảnh ⇒ **không suy lại được từ seed**, phải TRA. Cửa tra theo khoá điều khiển **nay đã có** — `POST /identity/resolve-by-controller` gộp qua Database PR #171 (2026-08-12), issue #162 đóng cùng ngày; đo 14/08 trên `origin/main` kho Database: `ResolveByControllerService.java` + `ResolveByControllerDtos.java` + `V38__taad_keys_pubkey_index.sql` + test đều có. Chặn còn lại ở phía app: màn khôi phục vẫn bắt gõ tay chuỗi `did`, chưa gọi cửa này. Rail guardian thì MỞ — `state_nft_logic.is_fresh` ép `guardians >= 2` ngay ở genesis nên không DID nào thiếu rail, và `FinalizeRecovery` cài `device_pkh` mới. Chặn còn lại: `rust_core` dựng genesis với `guardians` rỗng ⇒ tx tạo DID sẽ bị từ chối sau redeploy (Validator #77), và ký guardian còn đòi dán seed guardian vào máy |
