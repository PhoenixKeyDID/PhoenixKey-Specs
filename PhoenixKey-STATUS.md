...
| Tạo **ServiceDID / AgentDID / DeviceDID** | **KHÔNG** | 0 dòng backend — `/devices/register` (`DeviceController`) là push-token FCM/APNs, KHÔNG tạo DeviceDID; `grep` `DidType.DEVICE` = 0. Validator sẵn sàng ở tầng `can_own`, nhưng **client cũng chặn**: `rust_core` `build_create_taad_utxo_tx` chỉ nối đường Person; đường `GenesisChild` (`build_create_child_taad_utxo_tx`) còn datum 10 trường và redeemer cũ, không lớp PA2, trong khi `genesis_uniqueness_ok` phủ CẢ Child (R5) ⟹ ngay cả khi backend chuyển sang đúc anchor thật, hôm nay chỉ dựng được tx cho Person. Đường Person thì đã đi hết sáu cổng (redeemer, did do builder sinh, `genesis_ms` neo `upper_bound`, datum 15 trường, `device_pkh` + guardian, ExUnits đo thật) — `PhoenixKey-Core` #80 |
