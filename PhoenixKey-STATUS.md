...
3. 🔴 **Chưa có mốc chuyển.** Đợt redeploy mang tên PA-1 là chỗ v1.0 → v1.1 xảy ra. Hai việc trong danh sách chặn đã xong: schema `TAADDatum` là **16 trường** (`types.ak`, trường cuối `aux_device_pkhs` thêm ở `f2abc8c`) và `taad_did.rs` đã lên đủ 16 cho cả nhánh Person lẫn nhánh con (`PhoenixKey-Core` #81, `cargo test` 538/0). Còn lại: chọn `uniqueness_bootstrap_seed`, xử hai UTxO Preview, nạp tiền ví ký Preprod (§Deploy) — cộng thêm khuôn `did` (`PhoenixKey-Validator` #78, OPEN từ 2026-08-12, hỏi thẳng *"bao giờ luồng thật chuyển sang đúc anchor qua `state_nft_logic`"*).
...
**Điều kiện tiên quyết bị bỏ quên — khuôn `did` lệch.** Khuôn canonical DUY NHẤT là **PoP-bind** (`pop_bind.ak`: tiền-ảnh `enc_type ‖ enc_creator ‖ be8(genesis_ms) ‖ rand_256 ‖ controller_pkh`, mốc POSIX-**ms** render **base32 8-byte-BE ⟹ 13 ký tự**, hex thường). Backend và frontend vẫn sinh did theo khuôn CŨ đã bỏ (`DidPhoenixGenerator.java:194-206`, `PhoenixKey-Frontend/src/lib/sdvc/did.ts:128` `deriveDid`): mốc là **slot**, creator nối thô hoặc literal `"root"`, và **không có `controller_pkh`** trong tiền-ảnh. Hai khuôn CÙNG hình dạng chuỗi (`[a-z2-7]{13}` + 64 hex) nên regex không phân biệt được — lệch chỉ lộ ở giá trị hash. Bản Java theo khuôn mới **có mã nhưng đang tắt**: `application.yml:178 pop-bind-encoder-enabled: false`. Phía Rust không còn khuôn cũ: bộ sinh `construct_did` / `construct_did_with_rand` đã bị **xoá khỏi crate** (`PhoenixKey-Core` #80), bộ sinh canonical duy nhất nay là `rust_core/src/pop_bind.rs` — đối byte với vector Aiken (`docs/vectors/canonical_did.json`), **đã nối FFI**, builder tự sinh did và neo `genesis_ms` vào `upper_bound` của chính giao dịch. ⟹ **mọi DID cấp hôm nay không mint được anchor dưới validator hiện hành**, cho tới khi các đường gọi chuyển hết sang bộ sinh mới. Lập luận "đổi khuôn = đổi địa chỉ ví của mọi DID đang sống" **không còn hiệu lực**: chưa DID nào ra mainnet — `deploy/plutus-preprod.json` mang `"_stale": true` và hash `taad` của nó có trước cả hai đợt vá gần nhất, tức `pop_bind` chưa từng chạy trên chuỗi. Đường còn lại theo họ `DID-TS-UPPERBOUND` / `DID-CTRL-PKH` (`PhoenixKey-Database` #229) và `GENESIS-CHILD` (`PhoenixKey-Core` #80).
...
| Tạo **ServiceDID / AgentDID / DeviceDID** | **KHÔNG** | 0 dòng backend — `/devices/register` (`DeviceController`) là push-token FCM/APNs, KHÔNG tạo DeviceDID; `grep` `DidType.DEVICE` = 0. Validator sẵn sàng ở tầng `can_own`. **Client nay hết chặn**: đường `GenesisChild` (`build_create_child_taad_utxo_tx`) đã viết lại trọn ở `PhoenixKey-Core` #81 — redeemer đủ hai trường (`owner_did`, `rand_256`), `child_did` do builder sinh qua pop_bind, `genesis_ms` neo `upper_bound`, datum 16 trường có guardian + `device_pkh` + `depth`, và có chi shard-thread PA2 (`genesis_uniqueness_ok` phủ CẢ Child, R5). 27 test phủ 14/14 mệnh đề đọc được ở `state_nft_logic.ak:179-211`; `cargo test` 538/0. Đường Person cùng đợt đó lên 16 trường — trước đó nó dựng 15 trong khi `types.ak` đã 16 (`aux_device_pkhs`, `f2abc8c`), tức **cả hai nhánh đều đang chết ở `expect datum: TAADDatum`**. Còn chặn: 0 dòng backend, và chưa chạy tx thật lên node (chưa có `/utils/txs/evaluate` + submit Preprod) |
