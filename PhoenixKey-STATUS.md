...
| **Feecover trả hộ phí** cho AladinWork / OriLife | **KHÔNG** | 0 validator Feecover — `grep -rl "FeecoverAccrual\|FeecoverGate\|ServiceFeeSchedule\|FeecoverEpochSettle" --include='*.ak'` trong kho Validator = **0**; 4 chỗ có chữ `feecover` đều là chú-thích `[FEECOVER-HOOK]` chừa chỗ nối (`types.ak:360-365,394-398`, `taad.ak:28-29,124-129`, `taad_logic.ak:108-116`, `smartsend_logic.ak:109-110`). `feecoverAdvance` (FEECOVER-PRICE-2) chỉ là tên hàm trong spec — `grep -rn feecoverAdvance` trên toàn `PhoenixKeyDID` + `MagicLampEco` = **0 tệp mã**. Backend: `grep -rln "feecover\|consumemagic" -i` trên `PhoenixKey-Database/src` = **0**. Thêm nữa, phần "một ví đứng ra trả hộ phí mạng" **chưa được thiết kế ở tài liệu nào** — Feecover Tech §10 chỉ định tuyến CARP |
