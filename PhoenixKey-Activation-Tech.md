# PhoenixKey — Activation (Đặc-tả KỸ-THUẬT) · v4.1 (2-PHA GATED + forfeit)

> **Đối tượng:** KỸ SƯ triển khai (Tuân on-chain, Long backend, Core/Enclave, MAGIC/CARP/Registry-team).
> **Mục đích:** kiến-trúc + datum/redeemer CBOR + luồng tx + API backend + ranh-giới + thứ-tự deploy — đủ để code, không cần đọc lại spec Feat-Math.
> **Nguồn đối-chiếu (đã verify CODE THẬT):**
> - `PhoenixKey-Validator/validators/activation_vault.ak` (validator v4.1, 197 dòng)
> - `PhoenixKey-Validator/lib/phoenixkey/activation_logic.ak` (toán + invariant, 1859 dòng, 69 test)
...
└───────────────┬──────────────────────────────────────────────────────────────────────┘
                ...
                ▼
┌─────────────────────── BACKEND (PhoenixKey-Database, Java — Long) ────────────────────┐
...
- **own_policy ≡ script-hash** (multi-purpose, mẫu `taad`/`state_nft`): vault-NFT policy CHÍNH là hash validator → validator PHẢI có `mint` handler, nếu không mọi tx Mint rơi `else→fail`. Neo: `activation_vault.ak:80-83`, `activation_logic.ak:696-701`.
...
Neo: `activation_vault.ak:71-79`. Backend apply 7 tham-số → sinh script-hash + address RIÊNG mỗi DID (mẫu did_payment per-DID):
...
- **Ký:** engine Gen (interface MVP — §3.7 chưa chốt cơ-chế MAGIC). **Điều-kiện tối-thiểu load-bearing = "LAMP không rời qua GenDrip"** (I-ACT-7). Khi §3.7 chốt: thêm điều-kiện đọc/ghi `magic_batches`.
...
- **Ký:** keeper (system-authority MVP; `extra_signatories.has(keeper_pkh)`, `activation_vault.ak:106`).
- **Shape tx:**
  ```
...
- **Ký:** keeper.
- **Đo idle = GAP (LAZY, không counter tăng-dần):** `last_tick_epoch` chỉ tiến qua VestToOwner. Genesis=0 ⟹ user CHƯA vest lần nào forfeit khi `p2_epoch ≥ 1001`. **Backend tính idle từ `last_tick_epoch` (gap), KHÔNG dựa `idle_epochs_p2` (chỉ audit).** Neo: `activation_logic.ak:267-277`.
- **Shape tx:**
  ```
...
### 4.2 Gen (MAGIC yield — nền, ngoài Activation API)
```
engine /CARP đọc VaultDatum(conditional+vested) qua reference-input → drip MAGIC → magic_batches
KHÔNG spend LAMP. [BLOCKER §3.7 — nếu spend+recreate: dùng GenDrip redeemer, ép LAMP-preserved]
```
...
## 5. API backend (tham-chiếu API-for-SuperApp.md)

Prefix `/api/v1`, body snake_case, `DataResponse<T>{code,message,result}` (`code=1000`=OK). Chuyển-giá-trị theo mẫu **build-unsigned → Enclave witness → submit**.
...
| 2 | GET | `/activation/vault/{did}` | (đọc datum) | public | Long |
...
| **Registry-team** | chuẩn danh-mục dịch-vụ-tiêu-tài-nguyên-thật + cổng duyệt → keeper attest active/idle | Registry-team | **BLOCKER I-ACT-3** |
| **CARP-team** | GreenBack settlement interface (backed-path) + shadow-price | CARP | interface-only |
| **LAMP-team** | nạp pot lần-đầu (`dist_treasury`) + `fee_refill_lamp` phản-chu-kỳ + cân-đối dòng-vest-ra | LAMP + Long | tham-số §6.5 |
...
9. **fee_refill_lamp + cân-đối dòng-vest-ra** — chờ LAMP + Long (§6.5, sub-open pot cạn).
...
- **+ ca v4.1** (gated-per-epoch + forfeit gap + close-burn nhánh).
...
## 9. Ghi trung-thực (blocker + gap cần chốt)

- **§3.7 [BLOCKER MAGIC/CARP]:** engine Gen đọc-số-dư CHƯA spell-out on-chain. Validator ép tối-thiểu "LAMP không rời qua GenDrip". Khuyến-nghị: Gen dùng **reference-input** (đọc VaultDatum, KHÔNG spend) → drip magic_batches; nếu cần spend → GenDrip đã sẵn ép LAMP-preserved.
- **I-ACT-3 [BLOCKER Registry]:** keeper attest active/idle = MVP; cần Registry chuẩn dịch-vụ. Không có → anti-idle/vest-gate không phân-biệt tiêu-thật. Testnet chạy keeper-thủ-công tạm.
- **AbandonPhase1:** spec nêu, code v4.1 CHƯA có → endpoint 4c mặc-định BỎ (anti-idle tự thu-hồi).
- **plutus.json:** đã v4.1-current (không cần rebuild); chỉ comment `.ak` "7-field" lỗi-thời → Tuân sửa.
- **Pot cạn dòng-vest-ra PHA-2 (F, sub-open §6.5):** LAMP-vest rời-hệ → pot cần bù phản-chu-kỳ + treasury. Cân-đối tốc-độ chờ tham-số.
- **Ranh-giới sửa code:** validator (Tuân) + backend (Long) thuộc PhoenixKey backend — Claude KHÔNG sửa; phát-hiện lỗi → báo / Issue giao Long/Tuân. Tài-liệu này chỉ đặc-tả.
```
