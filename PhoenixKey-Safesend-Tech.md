# PhoenixKey — Safesend — Kỹ thuật (Tech)

> **Module:** Safesend (gửi-có-bảo-vệ). **Loại doc:** Kỹ-thuật cho implementer (đội on-chain / đội backend / Core rust_core / VeData-Glint). **Ngày:** 2026-07-09.
...
>
> **Ranh giới (MECE):** module CHỈ đặc-tả hòm ký-quỹ `safesend_escrow` (Open/Cancel/Accept/Finalize/Freeze/ResolveFreeze/ReclaimTimeout). **Cổng chi `did_payment`, guardian-recovery, anti-drain `limit_meter`** thuộc module Rebirthme — chỉ dẫn-chiếu. Xem [PhoenixKey-Safesend-Math.md](./PhoenixKey-Safesend-Math.md) cho bất-biến; [PhoenixKey-Rebirthme-Tech.md](./PhoenixKey-Rebirthme-Tech.md) cho ví/guardian/anti-drain; [PhoenixKey-Math.md](./PhoenixKey-Math.md) §10/§11 cho TAAD.
...
│  UI: màn Gửi (công-tắc Safesend) · Huỷ · Đồng-ý (Accept) · trạng-thái hòm     │
│  rust_core (FFI): safesend builder (Open/Cancel/Accept/Finalize) · glint proof │
└───────────────┬───────────────────────────────────────────┬───────────────────┘
                │ FFI                                        │ ZK proof
                ▼                                            ▼
┌──────────────────────────────┐              ┌────────────────────────────┐
│ PhoenixKey-Validator (Aiken) │              │  VeData / Glint (ZK)       │
│  safesend_escrow.ak           │◄── verify ───┤  Spectra + Glint context   │
...
Nền: hòm Safesend = **địa-chỉ script Plutus V3 stateful** (UTxO mang `SafeSendDatum`). Nguồn nạp = chi từ Ví Phượng-hoàng (`did_payment`, dẫn-chiếu Rebirthme). Guardian Freeze + factor Cancel dựa anchor TAAD (thuộc Core).

---

## 2. Bất-biến kiến-trúc (load-bearing)

1. **Escrow-UTxO đơn**: mỗi khoản Safesend = một UTxO tại `safesend_escrow`; mọi đường thoát ép `count(inputs ∈ Script(safesend)) == 1` (SS-7′) → không gộp hai hòm thoả-mãn một output.
...
### 3.2 `TAADDatum` (dẫn-chiếu Core Anchorme — field Safesend đọc)
Safesend ĐỌC (không sửa): `controller_pkh` (verify nguồn nạp + `receiver_commit` khi Accept), `guardians` (factor Guardian + SS-8/SS-8′ Freeze/ResolveFreeze quorum), enroll-set factor (SSR-4). Thứ-tự field khớp `types.ak` — thuộc Core/Validator, KHÔNG sửa ở đây.
...
|---|---|---|---|
| **Safesend Open** | `safesend_escrow` | tiêu UTxO `did_payment` → escrow; datum đủ (`open_slot`/`veto_deadline`/`reclaim_deadline`/`freeze_deadline`); `open_slot` trong validity-range tx Open, `window ≥ min_window_floor` (SS-10) | sender (controller) |
| **Safesend Cancel** | `safesend_escrow` | `now<veto_deadline` (cận-trên); ≥`factors_required` distinct-kind (đa-yếu-tố neo anchor SSR-4, khác gốc seed SS-6); `Σ→sender==amount+min_ada` | sender + factor |
| **Safesend Accept** | `safesend_escrow` | verify controller-sig `receiver_commit` qua anchor; chỉ set `receiver_consent=true`, mọi field khác + value bất-biến (SS-9′/SSR-14) | receiver (controller) |
| **Safesend Finalize** | `safesend_escrow` | `now≥veto_deadline` (cận-dưới); consent nếu `amount≥large_threshold` (trừ đích ngoài-Phoenix); `Σ→receiver==amount+min_ada` (SS-5′) | (permissionless / Bob) |
| **Safesend Freeze** | `safesend_escrow` | `now<veto_deadline`; guardian-sig hợp-lệ (neo anchor) → `frozen:=true`, chặn Finalize (SS-8) | guardian |
| **Safesend ResolveFreeze** | `safesend_escrow` | `frozen==true` ∧ (guardian-quorum m-of-n anchor.guardians ∨ `now≥freeze_deadline`) → `Σ→sender==amount+min_ada` (SS-8′) | guardian-quorum / permissionless sau `freeze_deadline` |
| **Safesend ReclaimTimeout** | `safesend_escrow` | `now≥reclaim_deadline ∧ receiver_consent==false`; `Σ→sender==amount+min_ada` (SS-11) | sender (permissionless sau hạn) |
| **Nạp nguồn (chi ví)** | `did_payment` | anchor Active + controller ký (dẫn-chiếu Rebirthme) | controller |

**Shape Open:** inputs = {vault UTxO(s) `did_payment`} ; outputs = {escrow UTxO(`SafeSendDatum`), [trả-lại-ví]} ; ref-inputs = {anchor} ; signer = controller ; validity `lo` = tip slot → `open_slot`.
...
| Endpoint | Việc |
|---|---|
| `GET /safesend/{did}/pending` | liệt-kê hòm mở (gửi đi / chờ nhận) + `veto_deadline`/`reclaim_deadline`/`freeze_deadline` |
| `GET /safesend/escrow/{utxo}` | trạng-thái một hòm (Open/consent/frozen) + đích |
| `POST /safesend/notify-accept` | thông-báo người-nhận có khoản chờ Accept |
...
| Team | Việc |
|---|---|
| **đội on-chain** | `safesend_escrow.ak` (SS-1..12, SSR-4) — 7 đường (Open/Cancel/Accept/Finalize/Freeze/ResolveFreeze/ReclaimTimeout). Dùng lại helper `anchor_controller_ok` + guardian-sig từ `auth_logic`/`taad_logic` (KHÔNG sao-chép). **KHÔNG sửa `did_payment.ak` mode-1.** |
| **đội backend** | index hòm mở + cửa-sổ-veto/reclaim/freeze + trạng-thái consent; thông-báo Accept; resolve đích. |
| **Core (rust_core/Flutter)** | builder Open/Cancel/Accept/Finalize/ResolveFreeze/ReclaimTimeout; công-tắc Safesend ở màn Gửi; màn Huỷ + Đồng-ý; hiển-thị cửa-sổ. Enforce factor Cancel khác gốc seed (I-CURVE-5, dùng chung Rebirthme). |
...
**Trình-tự onchain:**
1. **`safesend_escrow.ak`** — dựng hòm + 7 đường (Open/Cancel/Accept/Finalize/Freeze/ResolveFreeze/ReclaimTimeout) theo bất-biến SS-1..12 + SSR-4 đã hợp-nhất ở [PhoenixKey-Safesend-Math.md](./PhoenixKey-Safesend-Math.md) §4.
...
- **Chống-trộm phụ-thuộc anti-drain**: SS-6/T-SS-3 dựa `limit_meter.ak` (Rebirthme) làm nền — Safesend không phải lá-chắn chống-trộm đủ một mình khi thiếu lớp đó; xem tiền-đề (c) ở [PhoenixKey-Safesend-Math.md](./PhoenixKey-Safesend-Math.md) §6.
...
→ Trạng-thái & tiến-độ hiện tại: [PhoenixKey-STATUS.md](./PhoenixKey-STATUS.md#safesend)
...
Tài-liệu cùng bộ: [PhoenixKey-Safesend-Math.md](./PhoenixKey-Safesend-Math.md), [PhoenixKey-Safesend-Vi-Feat.md](./PhoenixKey-Safesend-Vi-Feat.md), [PhoenixKey-Safesend-Exec.md](./PhoenixKey-Safesend-Exec.md).
Code: `PhoenixKey-Validator/validators/safesend_escrow.ak`.
