...
> **Module:** Wakeme (kích-hoạt nhận LAMP). **Loại doc:** Feature (tiếng Việt, hướng người-dùng/sản-phẩm). **Ngày:** 2026-08-14.
...
> Một điều công bằng (mục-tiêu thiết-kế): **mỗi người chỉ một suất.** Đây là hướng đang xây, **chưa đạt đủ ở tầng hiện tại** — sinh-trắc trên máy chỉ đảm bảo một-suất-mỗi-máy, chưa chặn được một người dùng nhiều máy. Để đạt "một người một suất" thật, hệ dự-tính dùng nhiều lớp: xác-thực hai-yếu-tố, chứng-từ (Spectra), nhận-diện nhiều đặc-điểm trên ảnh/video khi có bản dùng cho người, và một lớp kiểm-tra trùng-người bằng công-nghệ mật-mã — lớp này **hôm nay chưa có ai nhận làm**, đang chờ đội phụ-trách chốt.
>
> Có một hiểu-lầm cần nói rõ: **không có chuyện quét một đoạn video selfie rồi dò khắp cả nước xem bạn đã đăng-ký chưa.** Lý do không nằm ở chỗ "mô-hình yếu", mà ở chỗ càng nhiều hồ-sơ để dò thì số lần nhận-nhầm càng cộng dồn — dò một triệu hồ-sơ nghĩa là chịu một triệu cơ-hội nhận-nhầm. Muốn giảm nhận-nhầm thì phải siết chặt tiêu-chuẩn nhận, mà siết chặt lại kéo theo từ-chối oan người thật. Số đo hiện có (trên cây, chưa từng đo trên người) cho thấy hai vế đó chưa cùng lúc đạt được: ở mức nhận-nhầm còn nghe được, đã có tới **gần ba phần tư** lượt người thật bị từ-chối. Công-nghệ nhận-diện nhiều đặc-điểm, khi có bản dùng cho người, sẽ đứng ở đúng chỗ của nó: **đối-chiếu một-đối-một** — người trước ống kính có đúng là người trong ảnh giấy-tờ bạn nộp hay không. Đó là một lớp kiểm-tra thêm, độc-lập với chứng-từ, chứ không phải cái cổng phân biệt bạn với cả triệu người khác.
>
> *Trong lúc các lớp này hoàn thiện, việc mở Wakeme rộng-rãi cho cá-nhân tạm hoãn. DID doanh-nghiệp/tổ-chức không dính vì đã có chữ-ký-cha xác thực (mỗi tổ-chức một OrgDID).*
...
- **"Một người một suất" (1 PersonDID/người, 1 OrgDID/tổ-chức):** D keyed theo PersonDID, không theo số ví — đa-ví/đa-DID không nhân suất. **Sinh-trắc-per-thiết-bị KHÔNG đủ** (chỉ 1-DID/máy). Uniqueness person-level thật cần nhiều lớp: 2FA + Spectra (chứng-từ, LampNet) + MCR (nhận-diện đa-đặc-điểm, OriLife — hôm nay chỉ dùng được ở vai 1:1, xem gạch đầu dòng dưới) + một kênh kiểm-tra trùng-người bằng công-nghệ mật-mã (từng dự-tính giao Glint) — lớp này **hôm nay chưa có ai nhận làm**, đang chờ chốt. Điều-kiện CẦN đã có phần: PA-2 UniquenessThread (SMT non-membership, đóng đúc-anchor-thứ-hai). Wakeme-PersonDID production chặn tới khi các lớp person-level land. DID doanh-nghiệp/tổ-chức không cần vì đã có chữ-ký-cha xác thực.
- **Nhận-diện đa-đặc-điểm trên ảnh/video — vai-trò đúng: đối-chiếu 1:1, KHÔNG phải cổng 1:N.** Bộ nhận-diện đa-đặc-điểm (MCR) hiện có trong hệ sinh-thái dựng cho **cây / quả / vật-nuôi**, chưa có luồng cho người và chưa có số đo trên người (kể cả số chống-giả-mạo trình-diện). Khi có bản dùng cho người, chỗ đứng của nó là **tầng độc-lập thứ hai cho phép so một-đối-một** (người trước ống kính ⟷ ảnh trong giấy-tờ đã nộp), cạnh kênh chứng-từ. Nó **không** dùng làm cổng dò-trùng 1:N trên toàn tập hồ-sơ: với sai-nhận mỗi phép so `f`, số hồ-sơ tối-đa còn dò được (ngân-sách sai 1%) là `N* ≈ 0,01/f`. Cần đọc `f` cho đúng — **`f` là một điểm vận-hành, không phải sàn của công-nghệ**: siết ngưỡng thì `f` nhỏ đi nhưng tỉ-lệ từ-chối oan người thật tăng lên, nên ràng-buộc thật là **cặp** hai số. Tại điểm vận-hành lỏng nhất còn nghe được trong bộ số đo (trên cây, `FAR 0,013` ⟹ `N* ≈ 0,77`), tỉ-lệ từ-chối oan đã là `0,73`. Về việc thêm kênh: gộp thêm kênh **cắt ra từ cùng một ảnh qua cùng một mạng** thì không nới được trần (bốn kênh như thế đo được chỉ đáng `1,16` kênh độc-lập); nhưng gộp một kênh **khác nguồn** thì có tác dụng thật — cặp khác-nguồn đo được `2,19` kênh hiệu-dụng trên 2 và hơn từng kênh riêng ở mọi mức sai-nhận. Điều đó cải-thiện phép so, vẫn không đủ để dựng cổng 1:N ở quy-mô dân-số. **Việc MCR còn đứng trong danh-sách lớp person-level hay không đang chờ chủ dự-án chốt** (Exec §6, Q-E). Chi-tiết + số đo: [Exec §7.1](./PhoenixKey-Wakeme-Exec.md).
