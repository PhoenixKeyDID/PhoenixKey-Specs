...
| **I-WALLET-7** | Chặn export nếu chưa có đường khôi-phục thay-thế ĐÃ DÙNG ĐƯỢC (guardian < 2 ∧ chưa có kênh khác đã bật) — không tự đẩy user vào ngõ-cụt. **Cập nhật 2026-08-30:** "VC-Glint" (Glint P-thr — quorum ẩn danh) hiện `[CONSTRUCTION-PENDING]`, verifier PHẢI reject `circuit_id` treo (`Glint-Math.md:196-199`) ⇒ **KHÔNG được tính là đường khôi-phục thay-thế cho tới khi P-thr hoàn-thiện** — guard này hôm nay trên thực-tế chỉ thoả bằng guardian ≥ 2 (hoặc Midnight khi tích-hợp xong). | UI-Spec §4/§5 (I-RECOVERY-4 grace 30 ngày); `Glint-Math.md:107,196-199,254` |
| **I-WALLET-8** | Khôi-phục = uỷ-quyền rotate, KHÔNG dựng lại Master_KEK; guardian KHÔNG giữ mảnh (đã bỏ Shamir). Đủ MỘT phương-thức DÙNG ĐƯỢC hôm nay (guardian / Midnight khi tích-hợp). **VC-Glint không tính vào "đủ MỘT" ở trên**: Glint cấp **P1 (knowledge-of-opening, sẵn có)** + **P-thr (quorum ẩn danh, `[CONSTRUCTION-PENDING]`)** cho Rebirthme (`Glint-Math.md:254`), nhưng cơ chế + chính sách recovery vẫn thuộc PhoenixKey (Glint KHÔNG build recovery) và P-thr chưa qua vòng hội-đồng construction — verifier PHẢI reject `circuit_id` treo (`Glint-Math.md:196-199`). **Mọi phương-thức đều phải đi qua sinh-trắc gắn DID** — chúng thay nhau ở lớp *uỷ-quyền*, không thay nhau ở lớp *nhận-dạng người*. | UI-Spec §0.2, §2, §6 — guardian-threshold, VC/Midnight; `INV-DEVICE`; `Glint-Math.md:107,174-183,196-199,254` |
