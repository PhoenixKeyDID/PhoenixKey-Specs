...
> **Mô hình: Triple Token** (LAMP · MAGIC · CARP) — KHÔNG phải Dual (2 token), KHÔNG phải one-token native (HopNhat lỗi thời). Nguồn canonical: `MAGIC/SPEC/MagicLamp-3Token-DacTa-Vi.md` (cập nhật 2026-07-01, ĐÈ phần một-token MAGIC của các bản trước).
...
- KHÔNG thiết kế oracle/buyback. KHÔNG chạm bất biến lõi CARP: **INV-PEG-ENDO** (`P*=1` theo `base_price`, oracle-free ở chuộc-par — `3Token §5`, `Carpet-CARP-DacTa-Vi.md`), **INV-BURN-EXCL** (đốt CARP theo backing, không theo peg), **`1 CARP = 1 MAGIC`** sức mua (bất biến hiến pháp) giữ nguyên.

Nguồn bám: `MAGIC-paymaster/ConsumeMAGIC/{CONTRACT.md, onchain/lib/magiclamp/consume/types.ak, onchain/validators/consume.ak}`; **`MAGIC/SPEC/MagicLamp-3Token-DacTa-Vi.md` (đặc tả Triple Token canonical — LAMP·MAGIC·CARP)**; `MAGIC/SPEC/Carpet-CARP-DacTa-Vi.md` (cơ chế ổn định CARP + 3 Gen); `VeDataIO/Specs/PhoenixKey-VeData-Contract.md v0.2.0 §2.1` (resolve API point-in-time).
...
Đây là câu hỏi bản chất. Tiền đề đúng: **1 MAGIC = một đơn vị sức mua dịch vụ nền, cố định danh nghĩa theo `base_price`** (`3Token §8.3`: "1 nanogic = 1 KB·ngày trên LampNet"; `3Token §1.2`: neo par `P* = 1`, KHÔNG neo fiat). MAGIC là **đơn vị-kế-toán sức mua ổn định theo thiết kế** — nó không "trôi theo giá thị trường" vì MAGIC không giao dịch, chỉ là số dư account trong Vault (non-transferable). Và CARP neo cứng `1 CARP = 1 MAGIC` (bất biến hiến pháp, `3Token §1.3`), nên **trả CARP theo `fee_magic` cũng ổn định-sức-mua như nhau**.

Do đó "ổn định dài hạn" của Feecover = **hai lớp**:

1. **Neo danh nghĩa (mặc định, zero-oracle):** `fee_magic` giữ NGUYÊN theo nanogic. Vì 1 nanogic ≡ 1 đơn vị sức mua nền, phí tự-ổn-định miễn là **giá dịch vụ nền** (`base_price`) không đổi. CARP trả = `fee_magic` thread, cùng sức mua. KHÔNG cần oracle, KHÔNG chạm INV-PEG-ENDO.
2. **Re-anchor bằng quản trị (khi `base_price` thực thay đổi):** khi DAO điều chỉnh `base_price` nền (`3Token §1.2`: base_price khoá on-chain, đổi chỉ qua DAO), **cả MAGIC lẫn CARP mua được nhiều hơn theo cùng tỷ lệ**, tỷ giá sức mua vẫn 1:1. Nếu chi phí hạ tầng thật của một dịch vụ PhoenixKey trôi khác nhịp `base_price` chung, DAO phát hành `ServiceFeeSchedule` mới (`version_epoch` tăng), điều chỉnh `fee_magic` của service đó. Đây là **quyết định con người theo cadence**, không phải hàm giá tự động bám giá ngoài (giữ INV-PEG-ENDO).
...
- Điều chỉnh `fee_magic` chỉ qua Governance (`status==Executed`, `gov_ref`). Biên mượn khung `base_price` (đổi qua DAO, thay đổi lớn cần siêu-đa-số — `3Token §1.2`, `Carpet-CARP-DacTa-Vi.md`) hoặc đặt riêng cho PhoenixKey — **chờ anh chốt §8**.
...
| FEECOVER-HO-3 | Feecover KHÔNG mint/burn CARP/LAMP; KHÔNG đọc oracle giá (giữ INV-PEG-ENDO ở tầng CARP) |
...
Nếu `base_price` nền trôi mà `fee_magic` giữ nguyên → phí lệch sức mua thực. → Re-anchor quản trị (§4.2 lớp 2), đồng bộ nhịp `base_price`. **Rủi ro:** cadence quá chậm. Giảm thiểu: cân "ổn định user" vs "provider không lỗ". **Không** giải bằng oracle real-time (giữ INV-PEG-ENDO). Lựa chọn có chủ đích: ổn định danh nghĩa > bám giá tức thời.
...
→ FEECOVER-HO-3 ép Feecover không mint/burn CARP/LAMP, không đọc oracle. INV-PEG-ENDO/INV-BURN-EXCL/`1 CARP=1 MAGIC` ở tầng CARP **không có input nào từ Feecover**. Kiểm tra: grep validator Feecover không import module CARP peg/oracle.
...
- Ổn định phí = neo danh nghĩa nanogic **trên sức mua MAGIC** (zero-oracle, giữ INV-PEG-ENDO) + re-anchor quản trị đồng nhịp `base_price`. KHÔNG dựa "MAGIC non-transfer", KHÔNG bám giá ngoài real-time.
...
- KHÔNG chạm `consume.ak` (C-CM-1..5) và KHÔNG chạm bất biến CARP-lõi (INV-PEG-ENDO, INV-BURN-EXCL, `1 CARP=1 MAGIC`).
