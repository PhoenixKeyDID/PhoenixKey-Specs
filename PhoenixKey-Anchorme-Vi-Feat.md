...
> Đặc tả toán ở PhoenixKey-Anchorme-Math.md (repo module nội bộ — private, chưa public); kỹ thuật ở PhoenixKey-Anchorme-Tech.md (repo module nội bộ — private, chưa public); điều hành ở [PhoenixKey-Anchorme-Exec.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-Anchorme-Exec.md).
...
Đáp: Bạn (hoặc OrgDID của bạn) đẻ **ServiceDID** con (GenesisChild). Sau này bàn giao được cho người khác qua **Transfer 2-of-2** (cả bên giao và bên nhận cùng ký). Đường tự phục vụ tạo ServiceDID qua API — chi tiết ở PhoenixKey-Anchorme-Tech.md (repo module nội bộ — private, chưa public).

**Hỏi: Chữ ký tôi ký năm ngoái, giờ tôi đổi chìa rồi, có còn kiểm được không?**
Đáp: Có — nhờ **point-in-time resolver**: hệ thống tra "tại thời điểm ký, chìa nào hợp lệ". Đặc tả đầy đủ ở PhoenixKey-Anchorme-Tech.md (repo module nội bộ — private, chưa public).
...
**Luật thiết kế:** tính độc nhất của con dấu Người phải được ép on-chain bằng một cơ chế cấu trúc riêng (**PA2 UniquenessThread**), không phải chỉ dựa vào khóa phần cứng. Song song, đường ví "đi theo-DID" thu hẹp bề mặt tấn công bằng **PA5-a entity-gate** (giới hạn loại thực thể được chấp nhận làm anchor tham chiếu). Đặc tả đầy đủ hai cơ chế này ở PhoenixKey-Anchorme-Math.md (repo module nội bộ — private, chưa public) §8-9 và PhoenixKey-Anchorme-Tech.md (repo module nội bộ — private, chưa public).

Các mảnh còn lại của hệ (resolve-by-hash, point-in-time resolver, DeviceDID, Authorization/Mint-Authority Registry, Permission & Consent) là đặc tả mở rộng thuộc ranh giới đội backend/on-chain — chi tiết ở PhoenixKey-Anchorme-Tech.md (repo module nội bộ — private, chưa public) §5-7.
