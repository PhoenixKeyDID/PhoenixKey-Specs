...
| **LMINT-7** | `threshold ≥ 2` cho model threshold (theo PR #40); `threshold ≤ size(member_dids)`. Vi phạm → lỗi cấu hình OrgDID, không phải lỗi mint (đã enforce ở founding). |
| **LMINT-8** | Mint hợp lệ CHỈ khi tx **spend SupplyState NFT** (redeemer `Advance`) + **mint route đúng** (`DistributionVest` cho đường OrgDID) + recreate SupplyState' cùng script address + `dist_minted + Δ ≤ dist_cap` (⇒ `minted_total ≤ TOTAL_CAP_OIL = 36 tỷ`). **Cap enforce ở validator SupplyState** (nguồn chân lý); backend chỉ fail-fast (`LAMP_MINT_CAP_EXCEEDED` / `LAMP_MINT_NO_SUPPLY_STATE`). Tx không compose SupplyState = hở cap → reject. |
