...
| Chuyển cho người khác được không | Không (gắn riêng bạn; không tích luỹ dồn kỳ — xem ghi chú dưới) | Có |
...
MAGIC không phải một "khoản đã nạp sẵn rồi hao dần" — nó được **tính lại mỗi kỳ** từ số dư LAMP hiện tại của bạn (đèn to → điều ước nhiều, đèn hao → điều ước ít theo), không cộng dồn phần đã bỏ lỡ từ kỳ trước sang kỳ sau ("dùng hay mất", không tích trữ). Cơ chế tính chính xác từng kỳ (engine Gen) **[CẦN CHỐT — 2026-07-12]** chưa chốt xong ở tầng validator (`PhoenixKey-Wakeme-Math.md` §3.7/§9); mô tả ở đây là nguyên tắc, không phải công thức cuối.
