...
Neo cố-định: **1 CARP = 1 MAGIC**. Một MAGIC được định-nghĩa bằng công-dịch-vụ nền (theo whitepaper: *1 MAGIC = 1 GB·ngày lưu-trữ lạnh*, tức `1 nanogic = 1 byte·ngày`), **không neo theo USD hay bất kỳ tiền pháp-định nào**. Đó là lý do phí giữ ổn-định theo sức-mua dịch-vụ chứ không nhảy theo giá thị-trường.
...
- **Ba phụ-thuộc kỹ-thuật bắt-buộc trước khi thanh-toán CARP chạy được (B1/B2/B3):**
  1. **B1 — Mô hình MAGIC** phải là số dư account trong Vault, non-transferable, KHÔNG policy-id.
  2. **B2 — Đồng CARP** phải có định-danh chính-thức trên chain (policy-id) trước khi luồng thanh-toán chạy được.
  3. **B3 — Gắn danh-tính vào từng lần tiêu phí** (`did_commit` per-DID): gom phí phải truy được về từng người thật, không chỉ ở mức "chủ ví" (owner).
...
- **Việc ổn-định-giá đồng CARP nằm NGOÀI Feecover** — do một lớp riêng của hệ (L3, GreenPeg/RedPeg) đảm-nhận. Feecover chỉ bàn-giao phần dư về Quỹ Phoenix.
