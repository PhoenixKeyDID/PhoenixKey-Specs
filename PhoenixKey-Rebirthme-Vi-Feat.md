...
### 4.7 Xuất seed — hai lựa chọn, bạn tự quyết
PhoenixKey **không bắt** bạn chép 24 từ (sao lưu tự động đã lo khôi phục). Khi bạn CHỦ ĐỘNG bấm Xuất seed, có hai lựa chọn tách bạch (chốt 2026-07-12, thay quy định cũ "luôn đổi khoá trước khi hiện"):

- **Xuất seed đang hoạt động** — hiện đúng cụm từ đang điều khiển DID của bạn (KHÔNG đổi khoá trước). Seed này dùng được ngay để nhập vào ví Cardano khác hoặc tự lưu trữ theo cách của bạn. Vì seed vẫn còn hiệu lực, lộ seed = lộ quyền kiểm soát tài sản thật — cụm từ được hiện theo kiểu **bấm mở từng từ một** (không hiện cả cụm cùng lúc dưới bất kỳ hình thức nào), chặn chụp/ghi màn hình khi màn đang mở, và bạn có thể **xoay khoá bất cứ lúc nào sau đó** một khi đã hiểu rõ mức an toàn của LampNet.
- **Xuất rồi vô hiệu (như trước)** — hệ thống đổi khoá + di tản tài sản TRƯỚC rồi mới hiện, cụm từ hiện ra đã bị vô hiệu với DID (chỉ để lưu niệm). Vẫn là lựa chọn mặc định gợi ý cho người mới.

**Xuất bản mã hoá để tự cất riêng:** ngoài xem trực tiếp, bạn có thể xuất seed dưới dạng **một tệp mã hoá bằng mật khẩu riêng do bạn đặt** (khác PIN ví) — mã hoá AES-256-GCM, khoá dẫn xuất từ mật khẩu qua PBKDF2/Argon2id, lưu về thiết bị/nơi lưu trữ riêng của bạn. Nhập lại tệp này kèm đúng mật khẩu để khôi phục khi cần.

**Nhập khoá tạo từ phần mềm ví khác:** PhoenixKey cho phép nhập một cụm mnemonic BIP39 chuẩn được tạo từ ví khác (không phải do chip PhoenixKey sinh ra) làm khoá điều khiển cho một danh tính — điều kiện: cụm từ phải hợp lệ theo chuẩn BIP39 (đúng checksum, đúng độ dài 12/15/18/21/24 từ).

**Trên web:** cùng năng lực xuất/nhập seed dự kiến có trên bản web, với cùng nguyên tắc bấm mở từng từ + không hiện cả cụm cùng lúc. `[CẦN CHỐT]` — bản web hiện chỉ giữ phiên đăng nhập qua quét mã từ mobile, chưa tự giữ khoá đã giải mã trong bộ nhớ trình duyệt; đưa năng lực xuất seed lên web đòi một quyết định kiến trúc riêng (khoá có bao giờ nằm trong bộ nhớ JS của trình duyệt hay không — bề mặt tấn công XSS khác hẳn mobile có Secure Enclave) — cần chốt trước khi triển khai trên web.
