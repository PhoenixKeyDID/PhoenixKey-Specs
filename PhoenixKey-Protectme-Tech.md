...
(so-sánh KIẾN-TRÚC minting policy — `lamp_policy.ak` còn sống trong `PhoenixKey-Validator`;
`magic_policy.ak` **đã bị xoá 2026-07-01** theo quyết-định MAGIC = account-trong-vault, giữ tên
ở đây làm ví-dụ lịch-sử cho mẫu authority-sig, KHÔNG phải file đang tồn tại).
