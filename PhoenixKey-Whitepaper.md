...
> **Dành cho ai tò mò sâu hơn:** mục tiêu dài hạn của PhoenixKey là một **Open SDK cho mọi đội xây dựng trên Cardano** (Cardano = một mạng sổ cái công khai, phi tập trung — không công ty nào một mình sửa hay xoá được dữ liệu đã ghi vào đó, giống một cuốn sổ công chứng ai cũng đọc được nhưng không ai một mình tẩy xoá được) — không chỉ một ứng dụng, mà một lớp hạ tầng danh tính + tài sản mà ai cũng cắm vào dùng lại được, giống vai trò Stripe cho thanh toán hay Auth0 cho đăng nhập, nhưng có sẵn ví + khôi phục bằng sinh trắc. Chuẩn kỹ thuật `did:phoenix` (xem giải thích "số CMND số" ở bảng từ điển mục 1.5 ngay dưới) đã đăng ký chính thức trong danh bạ DID Method của W3C — một danh bạ quốc tế do W3C (tổ chức chuẩn hoá web toàn cầu) quản lý, liệt kê các "kiểu danh tính số" đã được công nhận đạt chuẩn kỹ thuật chung. Đường dẫn kỹ thuật đầy đủ ở mục 6.
...
| Testnet (Preview) | Một mạng Cardano "phiên bản thử" tách biệt hoàn toàn với mạng thật (mainnet) — tiền và dữ liệu trên đó không có giá trị thật, dùng để thử trước khi phát hành ra mạng thật |

> **Vì sao 5/8 module có đuôi "-me" (Anchorme, Knowme, Rebirthme, Wakeme, Protectme) còn 3 module không có (Smartsend, Feecover, Easteregg)?** "-me" đánh dấu các module gắn trực tiếp và cá nhân hoá theo danh tính của riêng bạn (danh tính của bạn, giấy tờ của bạn, ví của bạn, kích hoạt của bạn, bảo hiểm cho bạn). Smartsend/Feecover/Easteregg là các lớp tiện ích dùng chung mọc lên từ ví, không mang tính "của riêng một người" theo cách đó.
...
**Đọc sơ đồ:** **Anchorme** là gốc — mọi thứ khác đều cần một danh tính đã neo trước. **Rebirthme** là chiếc ví gắn trực tiếp vào danh tính đó. Từ ví, các lớp mọc lên: **Smartsend** (gửi an toàn hơn), **Easteregg** (ẩn danh có kiểm soát), **Feecover** (trả phí gọn), và **Protectme** (lớp bảo hiểm phủ phần rủi ro còn sót — phủ cả tài sản trong ví Rebirthme lẫn sự cố liên quan phí Feecover). Song song, **Wakeme** (kích hoạt nhận LAMP) và **Knowme** (giấy tờ số) đều gắn thẳng vào danh tính, không phụ thuộc ví.

> Tên viết tắt trong sơ đồ — A (Anchorme), R (Rebirthme), S (Smartsend), F (Feecover), P (Protectme), E (Easteregg), W (Wakeme), K (Knowme) — tương ứng đúng 8 mục ở phần 4 bên dưới.
>
> **Trạng thái hôm nay:** cạnh Anchorme → Wakeme (kích hoạt nhận LAMP cho danh tính Cá nhân) hiện đang chờ một lớp bảo mật của Anchorme được xây nốt — xem [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-STATUS.md).
...
flowchart LR
    ...
    U2 --> U3["3. Gửi / nhận tiền<br/>bật Smartsend nếu cần"]
...
> **Trạng thái hôm nay:** bước 1–3 (tạo danh tính, có ví, gửi/nhận) đã chạy trên validator (173/173 test PASS); bước 4 (Nhận LAMP) và phần bảo vệ chống rút sạch trong bước 5 (khôi phục khi khoá đã lộ) đang trong lộ trình xây — xem chi tiết tại [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-STATUS.md).
...
> **Ghi chú đọc:** các mô tả dưới đây là **thiết kế đích** (kim chỉ nam). Một số cơ chế an toàn cốt lõi — van chống rút sạch của Rebirthme, hòm ký quỹ của Smartsend, cổng chi trả của Protectme — đang trong giai đoạn xây. Xem trạng thái thật hôm nay ở [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-STATUS.md) trước khi coi bất kỳ danh tính nào là an toàn cho giá trị lớn.
...
> (\*) Với danh tính **Tổ chức / Dịch vụ**, tính chất này đã đủ vì con dấu luôn đòi chữ ký của "cha" khi đúc (tạo ra lần đầu trên chuỗi — xem "Đúc (mint)" ở bảng từ điển mục 1.5). Với danh tính **Cá nhân**, một lớp tính độc nhất trên chuỗi đang được xây (đóng một lỗ hiếm gặp: kẻ xấu có thể đúc một danh tính mạo danh, trùng tên với bạn) — xem tiến độ ở [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-STATUS.md). Trước khi lớp đó xong, chỉ nên dùng danh tính Cá nhân cho việc không giữ giá trị lớn.

Xem sau: [Tính năng chi tiết](./PhoenixKey-Anchorme-Vi-Feat.md) · Toán/bất biến: PhoenixKey-Anchorme-Math.md (repo module nội bộ — private, chưa public)
...
- **Khôi phục qua người bảo chứng** (họ KHÔNG giữ mảnh bí mật của bạn) và **địa chỉ không đổi dù bạn xoay khoá bao nhiêu lần** — hai điểm này đã có trên validator. Số liệu test chi tiết theo thời gian thực xem [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-STATUS.md), không neo một con số cố định ở đây.
- **Một "van" chống rút sạch khi khoá lộ** — đang xây (xem STATUS.md). *Ví dụ:* nếu chìa khoá của bạn chẳng may bị lộ và có kẻ cố rút sạch trong một lần, "van" này giống cầu dao tự động ngắt khi phát hiện dòng điện bất thường — nó tự động chặn hoặc làm chậm giao dịch bất thường, cho bạn kịp thời gian phản ứng, thay vì mất hết trong một cú bấm. Tới khi van này xong, với ví giữ giá trị lớn hãy bật **Đóng băng ngay khi nghi lộ khoá**.

Xem sau: [Tính năng chi tiết](./PhoenixKey-Rebirthme-Vi-Feat.md) · Toán/bất biến: PhoenixKey-Rebirthme-Math.md (repo module nội bộ — private, chưa public) (mục §4.B nói về van chống rút sạch, và chứng minh mức thiệt hại tối đa — mức trần đó chỉ đúng KHI van đang bật) · Năng lực ví mở rộng — staking, quản trị Cardano, pool, SRCL: [PhoenixKey-Wallet-Staking-Governance-Vi-Feat.md](./PhoenixKey-Wallet-Staking-Governance-Vi-Feat.md) · Tự chọn nguồn đọc chuỗi (không khoá cứng vào 1 nhà cung cấp): [PhoenixKey-CustomBackend-Feat.md](./PhoenixKey-CustomBackend-Feat.md) · Kết nối trực tiếp ví với dApp/sàn Cardano (CIP-30, Minswap/JPG Store…): [PhoenixKey-DappConnector-Feat.md](./PhoenixKey-DappConnector-Feat.md)

### Smartsend — Gửi có bảo vệ, nút "hoàn tác" cho giao dịch
...
> **Lưu ý:** Smartsend là **tuỳ chọn (opt-in)** — nếu bạn KHÔNG bật, giao dịch gửi đi vẫn tức thời và **KHÔNG thể hoàn tác**, giống ví thường. Hãy bật Smartsend cho khoản tiền bạn thấy rủi ro; khoản nhỏ đời thường có thể gửi thường cho nhanh.

Cơ chế này đã có đặc tả toán học đầy đủ và đang trong giai đoạn xây code — theo dõi tiến độ ở [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-STATUS.md).
Xem sau: [Tính năng chi tiết](./PhoenixKey-Smartsend-Vi-Feat.md) · Toán/bất biến: PhoenixKey-Smartsend-Math.md (repo module nội bộ — private, chưa public) (12 bất biến kiểm chứng được đánh mã SS-1..12, và một định lý chứng minh "không ai mất tiền do lỗi cơ chế" — gọi tắt Định lý No-loss)
...
Xem sau: [Tính năng chi tiết](./PhoenixKey-Wakeme-Vi-Feat.md) · Toán/bất biến: PhoenixKey-Wakeme-Math.md (repo module nội bộ — private, chưa public)
...
Xem sau: [Tính năng chi tiết](./PhoenixKey-Feecover-Vi-Feat.md) · Toán/bất biến: PhoenixKey-Feecover-Math.md (repo module nội bộ — private, chưa public)
...
Mỗi lần bồi hoàn có một **mức trần theo sự cố** (incident cap) để giữ quỹ bền vững cho mọi người dùng; không đền trùng phần ví đã tự cứu được. Tiêu chí phân loại lỗi hệ thống-vs-sơ suất và các mức cụ thể đang được đội chốt — xem [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-STATUS.md).
Xem sau: [Tính năng chi tiết](./PhoenixKey-Protectme-Vi-Feat.md) · Toán/bất biến: PhoenixKey-Protectme-Math.md (repo module nội bộ — private, chưa public) (bất biến mức trần theo sự cố mã `I-PROT-INCIDENT-CAP`, và 3 quyết định tham số kinh tế còn chờ chốt đánh mã nội bộ PROT-4/PROT-10/PROT-11)
...
Xem sau: [Tính năng chi tiết](./PhoenixKey-Knowme-Vi-Feat.md) · Toán/bất biến: PhoenixKey-Knowme-Math.md (repo module nội bộ — private, chưa public)
...
Xem sau: [Tính năng chi tiết](./PhoenixKey-Easteregg-Vi-Feat.md) · Toán/bất biến: PhoenixKey-Easteregg-Math.md (repo module nội bộ — private, chưa public)
...
Nguồn thu: **phí Feecover** (chênh lệch quy đổi khi hệ thống trả phí mạng bằng CARP hộ người dùng) + phần **LAMP không được kích hoạt hết** chuyển về Treasury (kế toán, KHÔNG đốt bỏ). Mô hình phí license SDK cho đội thứ ba đang thiết kế — xem [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-STATUS.md).
...
Dự án đang ở giai đoạn testnet — chạy trên mạng thử Preview của Cardano (xem "Testnet (Preview)" ở bảng từ điển mục 1.5), chưa lên mạng thật (mainnet) — có audit nội bộ (red-team) chạy theo từng module. Trạng thái thật từng module — cái gì đã production-ready, cái gì còn chặn bởi blocker nào — cập nhật riêng, không trộn vào tài liệu định vị này.

Đây là kim chỉ nam thiết kế, không phải báo cáo tiến độ.
- Hiện trạng thật hôm nay: [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Anchorme-Specs/blob/main/PhoenixKey-STATUS.md)
