...
>
> → Trạng thái & tiến độ hiện tại: [PhoenixKey-STATUS.md](https://github.com/PhoenixKeyDID/PhoenixKey-Easteregg-Specs/blob/main/PhoenixKey-STATUS.md)
...
thấy số dư của bạn?") rồi tự dịch sang tầng thực thi. Có **5 mức hiển thị cho người dùng**
(Privacy-Mode, bảng dưới) dựng từ **4 cơ chế nền** ở bảng §3 phía trên — M1+ KHÔNG phải một
tầng độc lập thứ 5, mà là **biến thể của M1** (thêm bước "gom nhiều địa chỉ riêng thành một
nguồn thu ẩn danh" — cơ chế "Địa chỉ riêng" ở bảng §3 — cộng lên trên nền kho ẩn số dư của M1):
...
- **Nguyên tắc chống gian lận.** Hai validator, HAI bộ luật RIÊNG (đính chính 2026-07-12 — bản
  trước gộp nhầm chung một bộ, sai): validator giữ tiền (`did_pool` — mức "kho ẩn số dư") phải
  cưỡng chế **9 bất biến chống gian lận** (đánh mã nội bộ SHIELD-1..9 — 9 luật cụ thể như "không
  ai rút quá phần mình", "tổng sổ luôn khớp tổng tiền khoá trong hồ", "không ghi lệch khi gom
  tiền"). Validator địa chỉ riêng (`did_subaddr` — địa chỉ L3 của ví Phoenix, xem §3) phải cưỡng
  chế một bộ **4 luật KHÁC** (mã I-EGG-10..13 — "chỉ chủ DID gốc chi được mọi địa chỉ con", "mỗi
  địa chỉ con không nhóm được với nhau khi nhận tiền", "quyền chi sống qua đổi khoá/khôi phục",
  "TUYỆT ĐỐI không công khai ánh xạ địa chỉ con — chỉ chủ DID giữ"). Đặc tả đầy đủ từng luật của
  cả hai bộ ở `PhoenixKey-Easteregg-Math.md` §4.1 (SHIELD-1..9) và §4.2 (I-EGG-10..13). `did_pool`
  KHÔNG phải một "loại địa chỉ" — nó là pool kế toán dùng chung, dùng cấu trúc **Merkle-Sum-Tree** (chính là
  "quy tắc toán học buộc tổng trên sổ luôn khớp tổng tiền thật khoá trong hồ" đã nhắc ở FAQ §9 —
  một cây dữ liệu mật mã trong đó mỗi nút cha lưu kèm tổng số dư của toàn bộ nhánh con, nên chỉ
