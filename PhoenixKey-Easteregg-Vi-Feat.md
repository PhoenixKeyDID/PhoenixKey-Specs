...
- **Nguyên tắc chống-gian-lận.** Validator giữ tiền (`did_pool` — mức "kho ẩn số dư") và validator
  ...
  chống-gian-lận** (đánh mã nội-bộ SHIELD-1..9 — 9 luật cụ-thể như "không ai rút quá phần mình",
  "tổng sổ luôn khớp tổng tiền khoá trong hồ", "không ghi lệch khi gom tiền"... đặc-tả đầy-đủ
  từng luật ở `PhoenixKey-Easteregg-Math.md`) trước khi nhận tài sản thật. `did_pool` KHÔNG phải
  một "loại địa chỉ" — nó là pool kế-toán dùng chung, dùng cấu-trúc **Merkle-Sum-Tree** (chính là
