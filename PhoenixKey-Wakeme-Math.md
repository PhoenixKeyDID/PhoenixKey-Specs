...
| **I-ACT-3** | `active` = **(baseline) `counterparty ≠ owner`** ∧ **(cổng chính) tiêu qua dịch-vụ Registry tiêu-tài-nguyên-thật**. Lớp-đệm on-chain chặn self-dealing 1-DID; Registry là cổng chính (sybil ≥2-DID chỉ Registry chặn được). | **CỔNG CHÍNH ngoài on-chain MVP** — keeper attest (tin-cậy §8). **Baseline `counterparty≠owner`: `has_counterparty_consume` hiện placeholder `→False` (chưa nối) — nối enforce ở đợt PA-1** (đổi validator = script-hash mới). | `has_counterparty_consume:282`; dispatch `keeper_signed` |
...
| **6.1** | **Registry-chuẩn-dịch-vụ** (cổng chống-wash CHÍNH) — chi-tiết yêu-cầu ở T-5 (§8) — tiêu tài-nguyên thật. Baseline `counterparty≠owner` xếp DƯỚI (lớp-đệm on-chain, nối `has_counterparty_consume` ở PA-1), không thay. | Trung — không đụng 3 định-lý (keeper vẫn không đoạt tiền); đụng ĐỘNG-CƠ anti-idle (đếm active đúng). | Registry-team + PA-1 (validator) |
