...
> **Module:** Wakeme (GetLAMP — kích-hoạt nhận LAMP). **Loại doc:** Toán hình-thức. **Ngày:** 2026-07-09 · **Sửa lớn cơ-chế PHA-2:** 2026-07-12.
...
>
> **Doc = THIẾT-KẾ ĐÍCH (north-star):** đặc-tả này mô-tả cơ-chế PHA-2 **5-LAMP/epoch** đã chốt 2026-07-12 (xem changelog dưới). **Validator hiện-hành trên nhánh code còn ở mô-hình CŨ** (vest-ghi-sổ 1/ngày + ClaimVested + forfeit-1001-epoch-idle). Các neo `file:hàm:dòng` trong doc này TRỎ CODE CŨ — giữ để đối-chiếu lịch-sử tới khi đội on-chain rebuild `activation_vault.ak`/`activation_logic.ak` theo changelog. Khi rebuild xong, cập-nhật lại neo dòng + rename field/redeemer. **Trong lúc chưa rebuild, khi văn (mô-hình mới) ≠ code (mô-hình cũ) → VĂN thắng** (đây là chỉ-thị đích, không phải báo-cáo hiện-trạng).
>
...
## 0. Changelog cơ-chế PHA-2 (2026-07-12) — từ vest-ghi-sổ sang 5-LAMP/epoch
...
| Trục | Mô-hình CŨ (tới 07-11) | Mô-hình MỚI (chốt 07-12) |
|---|---|---|
| Nhịp PHA-2 | NGÀY (`n > last_tick_day`) | EPOCH (`e > last_tick_epoch`), 1 epoch = 5 ngày = 432.000 slot |
...
| Đơn-vị bước | 1 LAMP/ngày | `q = min(5, c)` LAMP/epoch (5 = 1 LAMP/ngày × 5 ngày/epoch — giữ nguyên nhịp vest cũ, gom theo epoch) |
| Field số-dư | `vested_unlocked` = số-dư đã-vest CÒN TRONG vault (load-bearing) | `owned_out` (o) = luỹ-kế LAMP đã chuyển RA ví (chỉ AUDIT, không còn là số-dư-trong-vault); vault chỉ giữ `conditional_lamp` |
...
- **(b) First-principles:** bỏ trạng-thái trung-gian "vested-in-vault" và redeemer ClaimVested ⟹ ít bề-mặt-tấn-công hơn (4 redeemer thay 5), bất-biến bảo-toàn đơn-giản hơn (`L(vault) = c`, không còn `c + v`).
...
**Bất-biến bảo-toàn KHÔNG vỡ:** với mỗi vault, `D = c + o + r` (initial-D = conditional-còn-lại + đã-sở-hữu-ra-ví + đã-reclaim-về-pot). Mỗi LAMP rời vault đi đúng một trong {pot, ví-Phoenix-DID}, không đường đốt (LAMP 36B không-burn giữ nguyên). Chứng-minh: §7.
...
| `c` | ℤ≥0 (oildrop¹) | `conditional_lamp` — LAMP khoá có-điều-kiện (chưa-sở-hữu), **= toàn-bộ LAMP còn trong vault sống** | `.conditional_lamp` |
| `o` | ℤ≥0 | `owned_out` — **luỹ-kế** LAMP đã chuyển RA ví Phoenix (đã-sở-hữu, ngoài vault); AUDIT chỉ-tăng. *(field 4; code cũ tên `vested_unlocked` — rename khi rebuild)* | `.owned_out` (cũ `.vested_unlocked`) |
| `r` | ℤ≥0 | `reclaimed_to_pot` — luỹ-kế về pot (audit, chỉ tăng): Reclaim PHA-1 + ReclaimEpoch PHA-2 | `.reclaimed_to_pot` |
| `td` | ℤ≥0 | `last_tick_day` — NGÀY tick gần nhất (monotonic, **PHA-1**) | `.last_tick_day` |
| `ie` | ℤ≥0 | `idle_epochs_p2` — AUDIT-field: luỹ-kế epoch idle đã ReclaimEpoch (KHÔNG load-bearing) | `.idle_epochs_p2` |
| `te` | ℤ≥0 | `last_tick_epoch` — p2-epoch cuối **ĐÃ XỬ-LÝ** (OwnEpoch active HOẶC ReclaimEpoch idle); monotonic PHA-2 | `.last_tick_epoch` |
| `L(x)` | ℤ≥0 | lượng LAMP-token thật trong Value `x` = `quantity_of(x, lamp_policy, lamp_name)` | `lamp_in` |
| `q` | ℤ≥0 (oildrop) | bước PHA-2 mỗi epoch = `min(5, c)` (5 = 1 LAMP/ngày × 5 ngày) | `epoch_unit`, `q = min(epoch_unit, c)` |
...
**Hằng (mô-hình mới — target rebuild):**
```
...
reclaim_unit     = 1             (PHA-1: 1 LAMP/ngày-idle → pot)
epoch_unit       = 5             (PHA-2: q = min(5, c) LAMP/epoch — 1 LAMP/ngày × 5 ngày)
d_cap            = 1001
phase1_last      = 1001          (PHA-1 "Giai đoạn Ngày": n ≤ 1001;  PHA-2 "Giai đoạn Kỳ": n ≥ 1002)
```
> **Đã BỎ** hằng `forfeit_idle_epochs = 1001` của mô-hình cũ (thu-hồi PHA-2 nay per-epoch, không chờ gap-1001). **Đã THÊM** `epoch_unit = 5`.
...
(SỔ-VALUE)      L(vault) == c
```
Vault sống chỉ giữ **conditional LAMP**. LAMP đã-sở-hữu ĐÃ RỜI vault (nằm trong ví Phoenix của DID) — `o` chỉ là bộ-đếm audit, KHÔNG cộng vào Value vault. Ép ở MỌI redeemer giữ-vault-sống qua `lamp_out_amt == d_out.conditional_lamp`. Genesis lập bất-biến: ép `lamp_locked == conditional_lamp` với `o = 0` ⟹ `L = c`.

**Bảo-toàn per-vault (thay cho `L = c + v` cũ):** đại-lượng luỹ-kế bảo-toàn là
```
(BẢO-TOÀN)      D == c + o + r      (initial-D = conditional-còn + đã-sở-hữu-ra-ví + đã-reclaim-pot)
```
Genesis: `c = D, o = 0, r = 0`. Mỗi redeemer giữ `Δc + Δo + Δr = 0` (§5). Từ (SỔ-VALUE) + (BẢO-TOÀN) suy ra 3 định-lý §7.

**Đơn-điệu (I-ACT-6):**
```
(MONO-c)   c chỉ GIẢM          (Reclaim −1 | OwnEpoch −q | ReclaimEpoch −q);  KHÔNG đường tăng
(MONO-o)   o chỉ TĂNG          (OwnEpoch += q)
(MONO-r)   r chỉ TĂNG          (Reclaim +1 | ReclaimEpoch += q)
(MONO-td)  td strict-tăng      (mọi tick PHA-1: n > td_in)
(MONO-te)  te strict-tăng      (mọi xử-lý PHA-2: e > te_in — mỗi epoch xử ĐÚNG một lần)
```
...
| ID | Bất-biến (hình-thức) | Cơ-chế-ép on-chain | Neo `hàm` (⚠ tên MỚI — code CŨ chưa rebuild) |
|---|---|---|---|
| **I-ACT-1** | Genesis: `c = D ∈ [1,1001] ∧ o = 0 ∧ r = 0 ∧ td = 0 ∧ ie = 0 ∧ te = 0 ∧ L(vault) = D ∧ did_commit ≠ ∅` | mint-gate GenesisVault ép ĐÚNG 1 NFT (name = owner_commit) + khuôn datum + `lamp_locked == conditional_lamp` | `genesis_vault_ok` |
...
| **I-ACT-4** | Reclaim (PHA-1 Ngày): `c′ = c − 1 ∧ r′ = r + 1 ∧ o′ = o ∧ td′ = n`; đúng 1 LAMP → pot; grace + monotonic + đủ-số-dư | 10 mệnh-đề (keeper, n≤1001, n≥grace, n>td, c≥1, sổ, sổ↔value, đích-pot, anti-drain) | `reclaim_ok` |
| **I-ACT-5** | Conservation toàn-cục: `Σ D_phát = Σ (→pot) + Σ (→ví-Phoenix-DID) + Σ(c vault-sống)`; per-vault `D = c + o + r`. LAMP không tự-sinh. | hệ-quả (SỔ-VALUE)+(BẢO-TOÀN) + mỗi redeemer đích-đúng (pot\|owner) — §7 Định-lý 1 | tổng-hợp; đích: `lamp_to_addr` |
...
| **I-ACT-7** | GenDrip LAMP-preserved: `c′ = c ∧ o′ = o ∧ L(vault′) = L(vault)`; mọi sổ + DID/mốc bất-biến | 8 mệnh-đề: conditional/owned/L bất-biến + sổ bất-biến + anti-drain | `gen_drip_ok` |
| **I-ACT-8** | **OwnEpoch** (PHA-2 Kỳ, ACTIVE, GATED): `e > te ∧ q = min(5, c) ∧ q ≥ 1` ⟹ `c′ = c − q ∧ o′ = o + q ∧ r′ = r`, `L(vault′) = L(vault) − q`, `q` LAMP → **ví Phoenix DID** (`owner_address`), `te′ = e ∧ td′ = n ∧ ie′ = ie` | 9 mệnh-đề: n>1001, keeper-attest-active, e>te (đơn-điệu), q=min(5,c), q≥1, sổ `Δc+Δo=0`, đích-owner, `L′=L−q`, anti-drain | `own_epoch_ok` (recreate) ; `own_epoch_close_ok` (c′=0 → burn) |
| **I-ACT-8b** | **ReclaimEpoch** (PHA-2 Kỳ, IDLE): `e > te ∧ q = min(5, c) ∧ q ≥ 1` ⟹ `c′ = c − q ∧ r′ = r + q ∧ o′ = o`, `L(vault′) = L(vault) − q`, `q` LAMP → **pot**, `te′ = e ∧ ie′ = ie + 1 ∧ td′ = n` | 8 mệnh-đề: n>1001, keeper-attest-idle, e>te, q=min(5,c), sổ `Δc+Δr=0`, đích-pot, `L′=L−q`, anti-drain | `reclaim_epoch_ok` (recreate) ; `reclaim_epoch_close_ok` (c′=0 → burn) |
...
∧ e > te_in                                    -- (MONO-te) epoch xử ĐÚNG một lần, tiến đơn-điệu
∧ q ≥ 1                                         -- còn conditional để giao (c_in ≥ 1)
∧ c′ = c_in − q  ∧  o′ = o_in + q  ∧  r′ = r_in -- sổ: Δc + Δo = 0
∧ te′ = e  ∧  td′ = n  ∧  ie′ = ie_in           -- mốc epoch đã-xử-lý
∧ identity_preserved
∧ L(out) = L(in) − q  ∧  L(out) = c′            -- q LAMP RỜI vault; còn lại = conditional
∧ lamp_to_addr(owner_address) ≥ q               -- ĐÍCH: ví Phoenix của DID (apply-param, DID-bound)
∧ nonlamp_preserved ∧ only_expected_policies    -- anti-drain
```
**Chú-ý auditor:** khác mô-hình cũ — LAMP RỜI vault THẲNG về ví Phoenix của DID (không còn "vest ghi-sổ trong vault" rồi ClaimVested). Đích cứng `owner_address` (apply-param) ⟹ keeper ký attest-active nhưng KHÔNG chọn được đích (không thể chuyển sang ví keeper). `q ≤ 5` là anti-drain per-epoch. Nhánh close (`own_epoch_close_ok`): `c′ = 0 ∧ ¬vault_recreated` → burn NFT (`close_vault_ok`). Neo: `own_epoch_ok` (recreate) ; `own_epoch_close_ok` (close).

### 5.3 ReclaimEpoch — `reclaim_epoch_ok` / `reclaim_epoch_close_ok` (PHA-2 Kỳ, IDLE)
```
let q = min(5, c_in)
keeper_signed                                  -- keeper attest epoch IDLE (Registry-gate §8)
∧ n > 1001                                     -- PHA-2
∧ e > te_in                                    -- (MONO-te) mỗi epoch xử ĐÚNG một lần
∧ q ≥ 1                                         -- còn conditional để thu (c_in ≥ 1)
∧ c′ = c_in − q  ∧  r′ = r_in + q  ∧  o′ = o_in -- sổ: Δc + Δr = 0 (owned BẤT BIẾN)
∧ te′ = e  ∧  ie′ = ie_in + 1  ∧  td′ = n       -- đếm audit idle-epoch
∧ identity_preserved
∧ L(out) = L(in) − q  ∧  L(out) = c′            -- q LAMP RỜI vault về pot
∧ lamp_to_addr(pot) ≥ q                         -- ĐÍCH pot (apply-param), KHÔNG ví keeper
∧ nonlamp_preserved ∧ only_expected_policies
```
**Khác mô-hình cũ:** thu-hồi MỖI epoch idle (tức-thời `q = min(5,c) → pot`), KHÔNG chờ gap-1001-epoch. `owned` (`o`) bất-biến (phần đã ra ví Phoenix không bị đụng — nhưng phần đó vốn đã NGOÀI vault). Nhánh close (`reclaim_epoch_close_ok`): `c′ = 0 ∧ ¬vault_recreated` → burn NFT. Neo: `reclaim_epoch_ok` (recreate) ; `reclaim_epoch_close_ok` (close).

### 5.4 GenDrip — `gen_drip_ok`
```
c′ = c_in ∧ o′ = o_in ∧ L(out) = L(in) ∧ L(out) = c′   -- LAMP đứng yên (I-ACT-7)
...
- **GenesisVault:** ĐÚNG 1 movement `+1` dưới own_policy; carrier output well-formed tại `Script(own_policy)`; `name == owner_commit`; khuôn I-ACT-1 (`o = 0`); `lamp_locked == conditional_lamp`. Neo `genesis_vault_ok`.
...
```
Chuyển hợp-lệ (đầy-đủ) — trạng-thái vault sống mô-tả bằng `c` (o,r là bộ-đếm audit ngoài Value):
- `GenDrip`: mọi pha, `c → c` — self-loop, `L(vault)` bất-biến (đọc `c` sinh MAGIC).
- `Reclaim`: chỉ PHA-1 (`n≤1001`), `c → c−1`, 1 LAMP → **pot**, `r += 1`.
- `OwnEpoch`: chỉ PHA-2 (`n>1001`), epoch ACTIVE + `e>te`, `c → c−q` với `q=min(5,c)`, q LAMP → **ví Phoenix DID**, `o += q`; đóng+burn nếu `c−q=0`.
- `ReclaimEpoch`: chỉ PHA-2, epoch IDLE + `e>te`, `c → c−q` với `q=min(5,c)`, q LAMP → **pot**, `r += q`; đóng+burn nếu `c−q=0`.

Chuyển BỊ CẤM (auditor cần xác nhận REJECT — test âm §… trong code sau rebuild):
`c → user` sai đích (OwnEpoch chỉ về `owner_address`) · L tự-sinh · Reclaim khi n>1001 · OwnEpoch/ReclaimEpoch khi n<1002 · `q > 5` (per-epoch cap) · `q > c` · xử-lại epoch `e ≤ te` (double-process) · non-keeper ký · OwnEpoch đích ≠ owner_address (keeper-đoạt) · ReclaimEpoch đích ≠ pot · đổi DID/mốc · drain ADA/token-lạ · burn khi c≠0.
...
**Chứng-minh.**
- Mô-hình mới KHÔNG giữ LAMP đã-sở-hữu trong vault: OwnEpoch chuyển `q` LAMP ra `owner_address` NGAY trong tx active-epoch (`L(out)=L(in)−q`). Ví Phoenix của DID là địa-chỉ user tự kiểm-soát — ngoài phạm-vi validator vault ⟹ keeper/anti-idle/reclaim không có redeemer nào đụng tới. Đây là bảo-đảm MẠNH hơn mô-hình cũ ("vested còn trong vault, bảo-vệ bằng owner-sig").
- Trong vault, `o` chỉ là bộ-đếm audit chỉ-tăng (MONO-o); ReclaimEpoch/Reclaim ép `o′ = o` (không giảm bộ-đếm). Không redeemer nào biến `o` thành LAMP rời-đích-pot.
...
> **Phát-biểu.** ReclaimEpoch chỉ hợp-lệ khi `n > 1001 ∧ e > te` (epoch mới, chưa xử-lý) ∧ keeper-attest-idle; hệ-quả `c → c−q` với `q=min(5,c)` toàn-bộ tới **pot** (không ví keeper), `o` bất-biến (phần đã ra ví không đụng), `r += q` (audit).

**Chứng-minh.**
- *Mỗi epoch xử ĐÚNG một lần (đơn-điệu):* guard `e > te_in` + hậu-điều-kiện `te′ = e`. Sau khi xử-lý epoch `e`, `te` nhảy lên `e` ⟹ mọi tx sau với `e_next ≤ te` bị REJECT. Không thể reclaim-lại một epoch ⟹ không double-forfeit. Vì `e` đơn-điệu-tăng theo thời-gian và `te′ = e`, chuỗi xử-lý tiến đơn-điệu, không xử-lại epoch cũ.
...
- *Chỉ về pot:* `L(out)=L(in)−q` **và** `lamp_to_addr(pot) ≥ q` (§5.3) ⟹ q LAMP tới pot (Định-lý 1, mục 3).
- *Chỉ phần chưa-kiếm:* `o′ = o` ⟹ LAMP đã ra ví không bị đụng (Định-lý 2). ReclaimEpoch chỉ rút từ `c` (conditional còn khoá).
...
1. **(SỔ-VALUE)** `L(vault) == c` giữ ở genesis + mọi redeemer? → grep `lamp_out_amt == d_out.conditional_lamp` (ở `gen_drip_ok`, `reclaim_ok`, `own_epoch_ok`, `reclaim_epoch_ok`). Per-vault `D = c + o + r`.
2. **Đích-đúng** mọi đường LAMP-rời: Reclaim/ReclaimEpoch → `pot_address`; OwnEpoch → `owner_address` (ví Phoenix DID); GenDrip → không rời. → `lamp_to_addr(...) ≥ threshold` + `only_expected_policies`.
3. **Chữ-ký:** Reclaim/OwnEpoch/ReclaimEpoch = keeper (attest active/idle). KHÔNG còn ClaimVested owner-sig — sở-hữu đi thẳng ra ví Phoenix của DID (đích cứng ⟹ keeper không đoạt).
4. **Ranh-giới pha:** Reclaim `n ≤ 1001`; OwnEpoch/ReclaimEpoch `n > 1001`. Test: Reclaim-PHA-2 reject, OwnEpoch-PHA-1 reject.
5. **Cap per-epoch** `q = min(5, c)` (không rút > 5/epoch). Test: q>5 reject, q>c reject.
6. **Đơn-điệu epoch** `e > te_in` + `te′ = e` (mỗi epoch xử ĐÚNG một lần, không xử-lại). Test: re-process `e ≤ te` reject.
...
## Phụ-lục A — Bảng đối-chiếu bất-biến ↔ hàm (mô-hình mới; neo dòng cập-nhật sau rebuild)

> ⚠ Neo dòng cụ-thể chờ đội on-chain rebuild `activation_logic.ak`/`activation_vault.ak` theo changelog §0. Bảng dưới liệt-kê HÀM đích của mô-hình mới.

| Bất-biến | Hàm (mô-hình mới) |
|---|---|
| days_elapsed / p2_epoch | `days_elapsed` / `p2_epoch` |
| (SỔ-VALUE) L=c ; per-vault D=c+o+r | genesis; gen; reclaim; own_epoch; reclaim_epoch |
| I-ACT-1 genesis khuôn (`o=0`) | `genesis_vault_ok` |
| I-ACT-4 reclaim (PHA-1) | `reclaim_ok` |
| I-ACT-7 gen-drip | `gen_drip_ok` |
| I-ACT-8 OwnEpoch (active, →ví Phoenix DID) | `own_epoch_ok` (recreate), `own_epoch_close_ok` (close) |
| I-ACT-8b ReclaimEpoch (idle, →pot) | `reclaim_epoch_ok` (recreate), `reclaim_epoch_close_ok` (close) |
| mint-gate genesis/close | `genesis_vault_ok` / `close_vault_ok` |
| dispatch spend/mint | `activation_vault.ak` (mint handler + spend `when redeemer`, 4 spend redeemer) |
