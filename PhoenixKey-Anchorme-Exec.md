...
>
> 🔴 **CỔNG GO/NO-GO (bản gốc, 2026-07-09):** Validator an-toàn về cấu-trúc, NHƯNG **KHÔNG mở tạo/custody danh-tính Người trên production tới khi PA2 UniquenessThread land.** Lý-do: GenesisPerson đúc được anchor-Person-giả cùng name với nạn-nhân + controller attacker (khóa phần-cứng P-256 không verify on-chain) → chiếm quyền, rút custody. Đây là lỗ **mã-hoá anchor**, KHÔNG phải lỗ sinh-trắc. Chi-tiết: [PhoenixKey-Anchorme-Math.md](./PhoenixKey-Anchorme-Math.md) §8-9.
>
> ⚠️ **ĐÍNH-CHÍNH 2026-08-13:** bản gốc còn viết thêm *"sinh-trắc Secure Enclave đã đủ chống trùng người"* — **mệnh-đề đó SAI, đã gỡ.** Khoá sinh trong vùng an-toàn của thiết-bị chỉ bảo-đảm **mỗi THIẾT-BỊ** tạo được một danh-tính; nó **KHÔNG** bảo-đảm **mỗi NGƯỜI** chỉ có một danh-tính — một người dùng nhiều thiết-bị vẫn tạo được nhiều danh-tính. Ràng-buộc một-người-một-danh-tính thuộc **Knowme** và neo vào **giấy-tờ tuỳ-thân**, không neo vào sinh-trắc ([PhoenixKey-Knowme-Math.md](./PhoenixKey-Knowme-Math.md) Đ-7, I-KNOW-12..16). Rủi-ro còn lại: R8 §3.
...
| **R8** | **Trùng-người ở mức NGƯỜI chưa đóng ở tầng anchor.** Một người dùng N thiết-bị đúc được N PersonDID đều hợp-lệ. PC + PoP-bind chặn *anchor thứ hai cho CÙNG một did-string*, KHÔNG chặn *người thứ nhất tạo did-string THỨ HAI* — hai bài-toán khác nhau. Khoá sinh trong vùng an-toàn của thiết-bị chỉ bảo-đảm mỗi thiết-bị một danh-tính, không bảo-đảm mỗi người một danh-tính. | 🔴 CAO (mức này trước đây bị hạ nhầm vì tin rằng sinh-trắc đã chống trùng người) | Ràng-buộc một-người-một-danh-tính thuộc **Knowme** v1, neo vào **giấy-tờ tuỳ-thân** — một giấy-tờ ⇒ nhiều nhất một PersonDID (`PhoenixKey-Knowme-Math.md` Đ-7, I-KNOW-12..16), KHÔNG neo vào sinh-trắc. Các lớp person-level còn lại + cổng chặn production: `PhoenixKey-Wakeme-Exec.md` §7. |
...
- **🟢 Lỗ R1 (CID-1) là lỗ MÃ-HOÁ ANCHOR, KHÔNG phải sinh-trắc/sybil — ĐÃ ĐÓNG 2026-08-12.** Lỗ gốc: GenesisPerson đúc anchor did-string bất-kỳ + controller attacker vì HW_Key P-256 không verify on-chain → drain custody (mọi loại DID, không riêng Person — đính-chính 2026-07-17). Validator on-chain ĐÚNG về cấu-trúc; lỗ nằm ở **giả-định-tầng-uniqueness chưa-thoả**, nay đã thoả: PC + PoP-bind đóng structural (did-string tự-chứng-thực), PA5-a (entity-gate) chốt KHÔNG cần nối vì dư thừa — chi tiết `PhoenixKey-Anchorme-Math.md` §8/§9.
- **🔴 Trùng-người ở mức NGƯỜI (R8) CHƯA đóng — mức rủi-ro trước đây bị hạ nhầm.** Bản trước của mục này ghi *"sinh-trắc Secure Enclave đủ chống trùng người"* rồi dựa vào đó coi sybil là chuyện đã xong. Mệnh-đề đó **SAI**: khoá sinh trong vùng an-toàn của thiết-bị chỉ bảo-đảm **mỗi thiết-bị một danh-tính**; một người dùng nhiều thiết-bị vẫn đúc được nhiều PersonDID đều hợp-lệ. PC + PoP-bind đóng "anchor thứ hai cho CÙNG một did-string", KHÔNG đóng "người thứ nhất tạo did-string THỨ HAI" — hai bài-toán khác nhau, đừng đọc kết-quả của cái này thành bảo-đảm cho cái kia. Ràng-buộc một-người-một-danh-tính thuộc **Knowme**, neo vào **giấy-tờ tuỳ-thân** (`PhoenixKey-Knowme-Math.md` Đ-7, I-KNOW-12..16); các lớp person-level còn lại và cổng chặn production: `PhoenixKey-Wakeme-Exec.md` §7.
