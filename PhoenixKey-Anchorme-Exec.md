...
| **R4** | **Type-code: văn Math lệch, impl KHÔNG lệch.** ĐÃ verify byte-exact: generator Java ≡ validator Aiken (Device=2…Service=7,Context=8). Chênh chỉ ở **văn Math §2.2** (thứ-tự Context=2). Rủi-ro: bản Rust/mobile bám nhầm văn Math. | 🟡 TRUNG | Person/Org/Character (0,1,9) TRÙNG → chưa lộ. **Chốt bảng-byte code làm canonical, sửa văn Math** (rẻ hơn rebake code); rà Rust/mobile trước khi mint author-DID phi-nhân (ProofChat v2). |
