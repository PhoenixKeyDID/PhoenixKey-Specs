# PhoenixKey — Protectme — ĐẶC TẢ ĐIỀU HÀNH (Exec)

> **Module:** Protectme (Bảo hiểm phủ tài sản) · **Loại doc:** Điều hành (lãnh đạo) ·
> **Ngày:** 2026-07-09.
> **Đối tượng đọc:** lãnh đạo — quyết định + rủi ro + lộ trình. Chi tiết kỹ thuật đọc
> [PhoenixKey-Protectme-Tech.md](./PhoenixKey-Protectme-Tech.md), toán/bất biến đọc [PhoenixKey-Protectme-Math.md](./PhoenixKey-Protectme-Math.md), người dùng đọc [PhoenixKey-Protectme-Vi-Feat.md](./PhoenixKey-Protectme-Vi-Feat.md).
>
> Bản này KHÔNG khoe sẵn sàng. Nó nêu rõ **11 quyết định + 3 blocker** phải chốt TRƯỚC
> production. Mọi tham số thành normative sau khi maintainer duyệt.
>
...
| Quyết định | Lý do (a · b · c · d) | Đánh đổi |
|---|---|---|
...
| **Beacon one-shot** chống double-payout | a chi trả kiểm soát được · b một claim = một lần chi, không tái · c on-chain enforce · d không cho lời gấp đôi | Cần validator hỗ trợ (đang chặn-merge) |
...
|---|---|---|---|---|
| 🔴 **Gán nhãn SYS lỏng = van pot-wide** (C.7 / VG-P2) | **NGHIÊM TRỌNG** | Một phán quyết SYS sai được KHUẾCH ĐẠI toàn-pot: batch tự phủ mọi DID "dính". Hợp lưu self-theft × thông đồng committee × drain | Evidence bar cứng (reproducer độc lập + postmortem-ký + phạm vi khớp chain) · vẫn soi từng-user trong batch · per-incident cap + pro-rata · challenge riêng cho nhãn | **CHỜ CHỐT [PROT-10]** — chưa có ngưỡng số. KHÔNG bật SYS-batch tới khi chốt |
| 🔴 **Cohort single-factor** (C.8 / VG-P3) | CAO | Solo một thiết bị: guardian/secondary cùng seed sập ĐỒNG THỜI khi lộ seed → chiết khấu cho phòng vệ ảo → rút ngầm pot_user | Định giá theo độc lập không theo hiện diện · bắt buộc anti-drain làm điều kiện phủ · trần thấp + loading · đẩy về cam kết bất khả huỷ | **CHỜ CHỐT [PROT-11]** — tiêu chí chứng thực độc lập chưa chốt |
| **Self-theft / claim gian lận** (C.2, C.6 / VG-P5,P6) | CAO | On-chain không phân biệt "trộm thật" vs "ví thứ hai của mình" | Challenge window + phạt nặng · cluster heuristic · đối chứng tín hiệu khôi phục · panel người | Chấp nhận gian lận LỌT ở mức trong I-PROT-SOLVENCY, không diệt tuyệt |
...
| **Adverse selection** (C.5 / VG-P8) | TRUNG | Chỉ user rủi ro cao mua → xoáy tử thần | Premium risk-based · T_wait_activate · bundle mặc định mức nền · no-claim bonus | Cần pool đủ rộng — nghiêng về bundle mặc định. Phía premium phụ thuộc Feecover |
...
| **Provenance escrow** (gap chưa chốt / VG-P1) | 🔴 MỞ | Beacon one-shot chưa có mã; ranh giới escrow-vs-Feecover B.6 chưa chốt | — | **GAP chặn-merge** — cần beacon + chốt cùng [PROT-6] |
...
> [N] chỉ thành normative sau khi maintainer duyệt.

### 4a. 11 quyết định PHẦN D chờ maintainer chốt (3 🔴 là van pot-wide)
...
| Blocker | Chặn gì |
|---|---|
| **MAGIC-model** (Account-trong-Vault, non-transferable) | Premium định giá không chạy tới khi Vault BurnBatch + Feecover accrual ổn |
| **CARP policy-id** | Payout + pot đơn vị CARP không settle được |
| **Beacon one-shot** (đội on-chain) | Chống double-payout chưa có on-chain enforce |

### 4c. Gap kỹ thuật

- **Beacon one-shot** (`protectme_beacon.ak`) — không có nó, một claim có thể chi nhiều lần
  (định lý solvency VỠ khi nạp trùng) → phải xong trước production.
- **Ranh giới escrow-vs-Feecover B.6 chưa chốt** — cần chốt cùng [PROT-6].
- Phần đội backend (hai bucket + Feecover wire + resolver claim) + UI Core/SuperApp là ranh
  giới đội khác, tài liệu này không code.

---

## 5. Lộ trình đề xuất

1. **Maintainer chốt 3 🔴 trước** (PROT-10, PROT-11, PROT-4) — chúng khoá bề mặt tấn công van pot-wide.
2. Chốt nốt 8 quyết định còn lại + 3 blocker hạ tầng (MAGIC-model, CARP policy-id, beacon).
3. Đội on-chain hoàn tất beacon one-shot → merge (chặn-merge kỹ thuật thuần duy nhất).
4. Đội backend wire hai bucket + Feecover premium + resolver claim (backend PhoenixKey = ranh giới đội backend).
...
**11 quyết định PHẦN D chờ maintainer** (3 🔴 van pot-wide: PROT-10 van gán-SYS, PROT-11 single-factor,
PROT-4 flaw-vs-negligence — bảng §4a). Bảy câu hỏi trọng yếu cần maintainer trả lời trước:

1. **Van pot-wide (PROT-10):** maintainer chấp nhận rủi ro "một nhãn SYS sai" tới mức nào? Đặt
...
2. **Single-factor (PROT-11):** có bắt buộc anti-drain làm điều kiện phủ USER cho DID
   single-factor không? (Đề xuất: CÓ — time-buffer là lớp thật duy nhất còn lại; `require_ad=True`
   đã cắm code, tunable.)
3. **Flaw-vs-negligence (PROT-4):** tiêu chí gán SYS (reproducer? audit-ký? phạm vi ≥N DID?) +
   ai nâng GREY→SYS? Gắn chặt câu 1.
4. **Phủ USER cao hay thấp (PROT-1/3):** phủ cao hơn = premium cao hơn nhiều + soi gắt hơn.
   Maintainer muốn nghiêng về niềm tin mạnh (đắt, rủi ro đạo đức cao) hay bền-pot (phủ hạn chế)?
5. **Bundle vs opt-in (PROT-7):** bật mức nền mặc định cho MỌI user (chống xoáy tử thần)
   hay opt-in thuần? Ảnh hưởng trực tiếp sống chết pot.
6. **KYC claim lớn (PROT-9):** đánh đổi giữa chống gian lận và không-token-weighted + riêng tư.
7. **Truyền thông:** chấp nhận nói thẳng với user "phủ tới hạn pot, sự cố cực lớn chia
...
- **Khối chi trả validator chỉ gác cái van cuối.** Mọi đòn gaming THẬT nằm ở tầng input datum
  (committee đặt `loss`/`cause`/`σ`/`α`) + phía NẠP (beacon + bucket-release) — đây là phần
  còn phải xây trước production.
- **Đây là bảo hiểm mà toàn bộ khâu định tổn thất + chống chi trùng vẫn là niềm tin
  off-chain** cho tới khi beacon + resolver + hai bucket có mã.
- Không hứa "an toàn tuyệt đối" hay "đền đủ mọi lúc". Self-theft là ranh giới nền tảng
  (không oracle mật mã) — chấp nhận gian lận LỌT ở mức trong I-PROT-SOLVENCY, có chủ đích.
