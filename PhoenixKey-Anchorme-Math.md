...
9. **(CID-1, ĐÃ ĐÓNG)** GenesisPerson đúc `did` tự-khai trùng nạn-nhân + ctrl attacker phải FAIL, không PASS? → xác-nhận qua `pop_bind_ok` (`state_nft_logic.ak:166`/`:192`) + `genesis_uniqueness_ok` (`taad.ak:89-92`) REJECT đúng theo thiết-kế. Chi-tiết: T-3 §8, CID-1 §9.
