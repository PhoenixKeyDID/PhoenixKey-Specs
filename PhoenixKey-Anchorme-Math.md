...
|---|---|---|---|
| `did` | ByteArray | chuỗi `did:phoenix:<genesis13>:<hash64>` (UTF-8); `<genesis13>` = base32 8-byte-BE của mốc POSIX-**ms**, KHÔNG phải slot | `types.TAADDatum.did` |
...
```
Đây CHÍNH là asset-name NFT on-chain (ép ở genesis) và là khoá tra `did_hash` off-chain (`phoenix_address.rs:52` `anchor_name_from_did`; `DID-Registry-Resolver-Feat §1.1`). KHÔNG salt, KHÔNG double-hash. Test-vector byte-exact trên did **canonical**: `blake2b_256("did:phoenix:aaaadgdcrqcaa:ff6bf9c8…8400") = 79712c5a…b544` (ca `person_root_2026`, `PhoenixKey-Validator docs/vectors/canonical_did.json`). Vector cũ `blake2b_256("did:phoenix:person:alice") = 1643c0c9…c470` (`…-Feat §1.3`) băm một chuỗi **dạng dev KHÔNG hợp khuôn canonical**; giữ lại chỉ để chứng minh `N` băm UTF-8 nguyên văn, không phân tích cấu trúc chuỗi.
...
| PA3 | Gate `did` prefix kiểu `did:phoenix:person:` on-chain | **BẤT-KHẢ-THI** với format DID thật: canonical = `did:phoenix:{base32(be8(genesis_ms))}:{hex_thường(inner_hash)}` (`pop_bind.ak` `canonical_did`), entity_type nằm TRONG tiền-ảnh (`enc_type`), KHÔNG lộ segment `:person:`/`:org:` để parse on-chain. Chuỗi `did:phoenix:org:acme` trong tài-liệu minh-hoạ là HƯ-CẤU, không phải format thật. |
...
```
inner_hash    = blake2b_256( enc_type ‖ enc_creator ‖ enc_genesis_ms ‖ rand_256 ‖ controller_pkh )
                --  1B      ‖ 1B gốc | 33B con ‖ 8B big-endian ‖ 32B ‖ 28B
canonical_did = "did:phoenix:" ‖ base32_nopad_thường(be8(genesis_ms)) ‖ ":" ‖ hex_thường(inner_hash)
                --                    ⟹ luôn 13 ký tự [a-z2-7]
...
| T-5 | **Type-code canonical: MÃ-VĂN lệch, KHÔNG lệch giữa impl.** Bảng-byte canonical LÀ chỉ-số constructor `types.EntityType`, pin ở `pop_bind.ak` `enc_type` (Person=0,Org=1,Device=2,Machine=3,Asset=4,Bot=5,Agent=6,Service=7,Context=8,Avatar=9); `DidPhoenixGenerator.java:56-65` khớp thứ-tự này. Văn Math §2.2 phải bám THEO bảng-byte này (KHÔNG dùng thứ-tự liệt-kê Person,Org,Context,Device,Machine,Asset,Bot,Agent,Service,Avatar). | Nếu ai encode sai thứ-tự bảng-byte → DID phi-nhân sinh khác. Rà bản Rust/mobile bám đúng bảng-byte trước khi mint author-DID phi-nhân. | maintainer + đội Core/backend §9 |
...
| **CID-4** | **Type-code canonical: BẢNG-BYTE code, không phải thứ-tự liệt-kê văn Math §2.2.** `pop_bind.ak` `enc_type` ≡ `types.EntityType` (Device=2…Service=7,Context=8) LÀ canonical. Văn Math §2.2 phải bám bảng-byte này (không dùng thứ-tự Context=2,Service=8). | Trung — chặn mint author-DID phi-nhân (ProofChat v2). Chưa lộ vì Person/Org/Avatar (0,1,9) trùng. Cần rà bản Rust/mobile bám đúng bảng-byte. | maintainer (Math v4.7) + đội Core/backend rà Rust/mobile |
