...
- `I-CANCEL-INDEPENDENT`: kênh Cancel/đóng-băng KHÔNG chỉ dùng `controller_pkh` cũ (ca mất-máy chính khoá đó không ký được) — thêm `ATTESTER_ALIVE` veto + kênh out-of-band (Glint P-thr — quorum ẩn danh, `Glint-Math.md:254`; **hiện `[CONSTRUCTION-PENDING]`, `Glint-Math.md:196-199` — verifier PHẢI reject `circuit_id` treo, nên kênh này CHƯA dùng được cho tới khi construction xong**). Vào `Recovering` PHẢI phát cảnh-báo out-of-band.
...
| **VeData / Glint (ZK)** | Cấp **P1 (knowledge-of-opening, sẵn có)** + **P-thr (quorum ẩn danh, `[CONSTRUCTION-PENDING]` — `Glint-Math.md:196-199`, CHƯA dùng được production)** cho kênh khôi-phục "VC-Glint"; Glint KHÔNG build cơ-chế/chính-sách recovery, việc đó thuộc PhoenixKey (`Glint-Math.md:174-183,254`). (Factor bối-cảnh Smartsend nay ở module Smartsend — [PhoenixKey-Smartsend-Tech.md §7](./PhoenixKey-Smartsend-Tech.md).) |
...
| S2 cấu-hình phương-thức khôi-phục | bật LampNet `recovery_anchor` / ngưỡng guardian / VC-Glint / Midnight | (đọc/ghi cấu-hình; VC-Glint CHƯA dùng được — P-thr `[CONSTRUCTION-PENDING]`, `Glint-Math.md:196-199`; Midnight tích-hợp sau) |
